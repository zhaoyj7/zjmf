  ...
  <com-config>
    ...
    <t-card class="list-card-container">
      ...
      <t-table row-key="id" :data="data" size="medium" @select-change="rehandleSelectChange"
        ...
        <template #notice_setting_name="{row}">
          {{row.notice_setting_name ? calcTitle(row.notice_setting_name) : '--'}}
        </template>
        <template #op="{row}">
    ...
    <t-dialog :visible.sync="visible" :header="addTip" :on-close="close" :footer="false" width="600"
      ...
      <t-form :rules="rules" :data="formData" ref="userDialog" @submit="onSubmit" v-if="visible">
        ...
        <t-form-item :label="lang.notice_default_aciton" name="notice_setting_name">
          <t-tree-select
            v-model="formData.notice_setting_name" :data="actionList" clearable filterable
