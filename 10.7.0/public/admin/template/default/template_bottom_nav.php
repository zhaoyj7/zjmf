      ...
      <div class="box">
        <div class="common-header">
          <div class="client-search">
            ...
            <t-button @click="handleGroup" theme="default" class="com-gray-btn">{{lang.temp_group_manage}}</t-button>
          </div>
        </div>
        <!-- table -->
        <t-enhanced-table row-key="key" ref="navTable" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :hide-sort-tips="true" drag-sort="row-handler" @drag-sort="onDragSort" :before-drag-sort="beforeDragSort" @expanded-tree-nodes-change="changeExpand" :tree="{ treeNodeColumnIndex: 1 }" :expanded-tree-nodes="expandedRowKeys">
          ...
          <template #show="{row}">
            <t-switch v-model="row.show" :custom-value="[1,0]" v-if="row.group_id" @change="(val)=>changeStatus(val,row)">
            </t-switch>
          </template>
          <template #blank="{row}">
            <t-switch v-model="row.blank" v-if="row.group_id" :custom-value="[1,0]" @change="(val)=>changeOpen(val,row)">
            </t-switch>
          </template>
          <template #op="{row}">
