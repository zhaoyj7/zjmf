  ...
  <com-config>
          ...
          <div class="nav_right">
            <div class="menu_set" v-show="isShowSet">
              <t-form :data="formData" label-align="top" :label-width="60" @submit="saveSet">
                ...
                <t-form-item v-show="formData.type == 'module' || formData.type == 'res_module'" name="is_cross_module"
                  :label="lang.is_cross_module" :help="lang.is_cross_module_tip">
                  <t-switch size="large" :custom-value="[1,0]" v-model="formData.is_cross_module"
                    @change="formCrossModuleChange"></t-switch>
                </t-form-item>
                ...
                <t-form-item v-show="formData.is_cross_module === 1" name="select_field" :label="lang.select_field">
                  <t-select v-model="formData.select_field" multiple :min-collapsed-num="1" filterable
                    :placeholder="lang.select_field_tip" @change="saveSet" clearable>
                    <t-option v-for="item in selectFieldList" :key="item.value" :value="item.value"
                      :label="item.label" />
                  </t-select>
                </t-form-item>
    ...
    <t-dialog :visible.sync="visible" :header="lang.add_page" :footer="false" @close="close" class="page-dialog">
      <t-form :data="newFormData" label-align="right" :label-width="120" @submit="confirmNewMenu" v-if="visible">
        ...
        <!-- 跨模块列表开关 -->
        <t-form-item v-show="newFormData.type == 'module'" name="is_cross_module" :label="lang.is_cross_module"
          :help="lang.is_cross_module_tip">
          <t-switch size="large" :custom-value="[1,0]" v-model="newFormData.is_cross_module"
            @change="newCrossModuleChange"></t-switch>
        </t-form-item>
        <!-- 模块类型: 合并本地+代理模块 -->
        <t-form-item v-show="newFormData.type == 'module'" name="url" :label="lang.module_type">
          <t-select v-model="newFormData.multiple" :min-collapsed-num="1" clearable @change="newModuleChange" multiple>
        ...
        <t-form-item v-show="newFormData.is_cross_module === 1" name="select_field" :label="lang.select_field">
          <t-select v-model="newFormData.select_field" multiple :min-collapsed-num="1" filterable
            :placeholder="lang.select_field_tip">
            <t-option v-for="item in selectFieldList" :key="item.value" :value="item.value" :label="item.label" />
          </t-select>
        </t-form-item>
        ...
        <t-form-item name="icon" :label="lang.icon_code" v-show="value=='2'">
          <t-popup placement="right-top" :visible="newBackPopupVisible" v-show="value=='2'">
            <t-input class="icon-input" v-model="newFormData.icon">
              <t-icon :name="newFormData.icon.replace('t-icon-','')" slot="prefix-icon"></t-icon>
            ...
            <template #content>
              <div class="all-icon">
                ...
                <div class="main-icons">
                  ...
                  <svg v-for="item in adminIcon" :key="item.icon_id" class="back-icons-item"
                    :class="item.font_class==formData.icon?'active ' + item.font_class: ''"
                    @click="newFormData.icon = item.font_class;">
                    <use :href="`#${item.font_class}`"></use>
                  </svg>
