  ...
  <com-config>
    <t-card class="list-card-container">
      <div class="common-header recyle-top">
        <div class="left">
          <span class="com-first-title">{{lang.order_recycle_bin}}</span>
        ...
        <div class="right-search">
          ...
          <t-button class="add" theme="danger" @click="handleRecyle('clear')" :disabled="data.length === 0" v-permission="'auth_business_order_recycle_bin_clear'">{{lang.clear_recycle_bin}}</t-button>
        </div>
      </div>
      <div class="common-header">
        ...
        <div class="right-search">
          ...
          <t-button @click="changeAdvance" theme="primary" variant="outline">{{isAdvance ? lang.pack_up : lang.advanced_filter}}</t-button>
        </div>
      </div>
      <!-- 高级搜索 -->
        ...
        <template #icon="{row}">
          <template v-if="row.type">
            <t-tooltip :content="lang[row.type]" theme="light" :show-arrow="false" placement="top-right">
              <img :src="`${rootRul}img/icon/${calcOrderIcon(row.type)}.png`" alt="" style="position: relative; top: 3px;">
            </t-tooltip>
          </template>
        </template>
      ...
      <div class="bot-opt">
        ...
        <t-button @click="handleRecyle('delete', true)" theme="danger" v-permission="'auth_business_order_recycle_bin_delete_order'">{{lang.batch_delete}}</t-button>
        <t-button @click="handleRecyle('lock', true)" theme="default" v-permission="'auth_business_order_recycle_bin_lock_order'">{{lang.order_lock}}</t-button>
        <t-button @click="handleRecyle('unlock', true)" theme="default" v-permission="'auth_business_order_recycle_bin_unlock_order'">{{lang.order_unlock}}</t-button>
      </div>
    </t-card>
    ...
    <t-dialog :header="lang.recycle_setting" :visible.sync="settingVisble" :footer="false" :width="isEn ? 700 : 480" class="recycleDialog">
      <t-form :data="recycleForm" ref="update_price" @submit="submitConfig" :rules="rules">
        ...
        <t-form-item :label="lang.storage_time" class="days">
          <span style="width: auto;">{{lang.order_exceed}}</span>
