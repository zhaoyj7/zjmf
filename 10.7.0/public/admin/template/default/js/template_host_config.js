  ...
  window.onload = function () {
    ...
    new Vue({
      ...
      computed: {
        ...
        calcMoreColumns () {
          if (this.curValue === "brand") {
            return this.baseColumns;
          } else {
            let temp = JSON.parse(JSON.stringify(this.moreColumns));
            temp.shift();
            return temp;
          }
        },
      ...
      methods: {
        ...
        /* 区域拖拽排序 */
        async onDragSortArea(params) {
          try {
            this.areaList = params.currentData;
            const arr = this.areaList.reduce((all, cur) => {
              all.push(cur.id);
              return all;
            }, []);
            const res = await changeAreaOrder(this.areaName, {
              id: arr,
            });
            this.$message.success(res.data.msg);
            this.getAreaList();
        ...
        /* 更多优惠拖拽排序 */
        async onDragSortMore(params) {
          try {
            this.moreList = params.currentData;
            const arr = this.moreList.reduce((all, cur) => {
              all.push(cur.id);
              return all;
            }, []);
            const res = await changeProductOrder(this.moreName, {
              id: arr,
            });
            this.$message.success(res.data.msg);
            this.getMoreList();
        ...
        /* 拖拽排序 */
        async onDragSortBase(params) {
          try {
            this.baseList = params.currentData;
            const arr = this.baseList.reduce((all, cur) => {
              all.push(cur.id);
              return all;
            }, []);
            const res = await changeProductOrder(this.name, {
              id: arr,
            });
            this.$message.success(res.data.msg);
            this.getBaseList();
