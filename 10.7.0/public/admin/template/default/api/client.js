...
function pushHostInfo(params) {
  return Axios.post(`/host/${params.id}/push_downstream`, params);
}
...
// 流量包列表
function getFlowPacketList(type, params) {
  return Axios.get(`/${type}/${params.id}/traffic_package`, { params });
}
// 订单内页产品退款列表
...
/* 订单信息记录列表 */
function getOrderRecordList(params) {
  return Axios.get(`/order/${params.id}/record`, { params });
}
function addAndUpdateOrderRecord(type, params) {
  if (type === "add") {
    return Axios.post(`/order/${params.id}/record`, params);
  } else if (type === "update") {
    return Axios.put(`/order/record/${params.id}`, params);
  }
}
function deleteOrderRecord(params) {
  return Axios.delete(`/order/record/${params.id}`);
}


// 获取实名认证
function getRealNameDetail(id) {
  return Axios.get(`/certification/client/${id}`)
}
