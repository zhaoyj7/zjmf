...
// 获取亏本交易拦截配置
function getLossInterceptConfig () {
  return Axios.get('/configuration/upstream_intercept');
}
// 保存亏本交易拦截配置
function saveLossInterceptConfig (params) {
  return Axios.put(`/configuration/upstream_intercept`, params);
}

// 手动同步代理资源
function handleSyncResource(params) { 
  return Axios.get(`/upstream/product/${params.id}/sync_resource`, {params});
}
