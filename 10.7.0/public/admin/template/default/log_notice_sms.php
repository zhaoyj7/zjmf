...
<div id="content" class="log-notice-sms" v-cloak>
  <com-config>
    <t-card class="list-card-container">
      ...
      <div class="flex">
        <t-tabs v-model="curTab" @change="changeTab" placement="left" style="flex-shrink: 0; margin-right: 20px">
          <t-tab-panel value="sms" :label="lang.sms_notice" key="sms"></t-tab-panel>
          <t-tab-panel value="email" :label="lang.email_notice" key="email"></t-tab-panel>
        </t-tabs>
        <div class="right-box">
          <div class="common-header" style="margin-top: 0;">
            <div class="left">
              ...
              <t-button class="com-gray-btn" @click="openExportDia"
                v-if="curTab === 'sms' && $checkPermission('auth_management_log_sms_log_export_excel') && hasExport">
                {{lang.data_export}}
              </t-button>
              <t-button class="com-gray-btn" @click="openExportDia"
                v-if="curTab === 'email' && $checkPermission('auth_management_log_email_log_export_excel') && hasExport">
                {{lang.data_export}}
              </t-button>
            </div>
            <div class="flex">
              <t-input v-model="params.keywords" class="search-input" 
                :placeholder="curTab === 'sms' ? `${lang.client_notice_sms_search}` : `${lang.client_notice_email_search}`"
                @keypress.enter.native="search" :on-clear="clearKey" clearable>
              </t-input>
              <t-button @click="search">{{lang.query}}</t-button>
            </div>
          </div>
          <t-table row-key="id" :data="data" size="medium" :hide-sort-tips="true" :columns="curColumns" :hover="hover"
