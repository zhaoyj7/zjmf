...
class Idcsmartali extends Plugin
{
    ...
    public function IdcsmartaliPerson($certifi)
    {
        ...
        if ($res1['status'] == 200){
            ...
            $url = $res2['url'];
            if (!empty($certifi['special'])){
                return ['url'=>$url,'certify_id'=>$certify_id];
            }
            ...
            $base64 = 'data:image/png;base64,' . base64_encode($response->getData());
    ...
    public function IdcsmartaliStatus($certify)
    {
        $certify_id = $certify['certify_id']??'';
        return (new IdcsmartaliLogic())->getAliyunAuthStatusSpecial($certify_id);
    }
}
