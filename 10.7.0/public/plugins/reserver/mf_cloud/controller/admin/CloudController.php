...
class CloudController
{
	...
	public function adminDetail(){
		...
		try{
			...
			// 操作系统
			$res = $RouteLogic->curl( sprintf('console/v1/product/%s/remf_cloud/image', $RouteLogic->upstream_product_id), $param, 'GET');
			if($res['status'] == 200){
                // 处理多级代理问题
	 ...
	 * @author hh
	 * @version v1
	 * @url /admin/v1/remf_cloud/:id/traffic_package
	 * @method  GET
	 * @param   int id - 产品ID require
	 * @param   int page - 页数
	...
	public function trafficPackageList()
	{
		...
		try{
			...
			$result = $RouteLogic->curl( sprintf('console/v1/remf_cloud/%d/traffic_package', $RouteLogic->upstream_host_id), [], 'GET');
		}catch(\Exception $e){
			if(!$RouteLogic->isUpstream){
				if(class_exists('\server\mf_cloud\controller\home\CloudController')){
