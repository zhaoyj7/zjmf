...
const app2 = Vue.createApp({
    ...
    return {
      ...
      powerStatusArr: {
        on: {
          ...
          icon: `/plugins/reserver/mf_cloud/template/clientarea/mobile/mfm301/img/cloud/on.svg`,
        },
        off: {
          text: lang.common_cloud_text11,
          icon: `/plugins/reserver/mf_cloud/template/clientarea/mobile/mfm301/img/cloud/off.svg`,
        },
        operating: {
          text: lang.common_cloud_text12,
          icon: `/plugins/reserver/mf_cloud/template/clientarea/mobile/mfm301/img/cloud/operating.svg`,
        },
        fault: {
          text: lang.common_cloud_text86,
          icon: `/plugins/reserver/mf_cloud/template/clientarea/mobile/mfm301/img/cloud/fault.svg`,
        },
        suspend: {
          text: lang.common_cloud_text87,
          icon: `/plugins/reserver/mf_cloud/template/clientarea/mobile/mfm301/img/cloud/suspended.svg`,
        },
      },
      // 显示重装系统弹窗
  ...
  methods: {
    ...
    handleBwRange(bwRule) {
      // 处理带宽范围判断
      // 优先使用 params 中的值，否则使用 cloudData 中的值
      let bwValue =
        this.params.bw !== undefined ? this.params.bw : this.cloudData.bw;
      // 如果值为 undefined，返回 true（不限制）
      if (bwValue === undefined || bwValue === null || bwValue === "") {
        return true;
      }
      const target = bwValue * 1;
      if (isNaN(target)) {
        return true;
      }
    ...
    handleFlowRange(flowRule) {
      ...
      // 优先使用 params 中的值，否则使用 cloudData 中的值
      let flowValue =
        this.params.flow !== undefined ? this.params.flow : this.cloudData.flow;
      // 如果值为 undefined，返回 true（不限制）
      if (flowValue === undefined || flowValue === null || flowValue === "") {
        return true;
      }
      const target = flowValue * 1;
      if (isNaN(target)) {
        return true;
      }
    ...
    handleIpv4Range(ipv4Rule) {
      // 优先使用 ipValue（用户当前选择的值），否则使用 cloudData.ip_num
      const ipv4Value = this.ipValue ?? this.cloudData?.ip_num;

      if (ipv4Value == null) return true;

      const target = Number(ipv4Value);
      if (isNaN(target)) return true;

      const min =
        ipv4Rule.min === "" || ipv4Rule.min == null ? 0 : Number(ipv4Rule.min);
      const max =
        ipv4Rule.max === "" || ipv4Rule.max == null
          ? Infinity
          : Number(ipv4Rule.max);
    ...
    async getLineDetails() {
      try {
        ...
        // 初始化 ipValue 为当前实例的 IP 数量，用于限制规则判断
    ...
    getManualOs() {
      manualResourceOs(this.id).then((res) => {
        if (res.data.status === 200) {
          ...
          this.osIcon =
            "/plugins/reserver/mf_cloud/template/clientarea/mobile/mfm301/img/mf_cloud/" +
            this.osData[0]?.icon +
            ".svg";
    ...
    doCheckImage() {
      ...
      checkImage(params).then(async (res) => {
        if (res.data.status === 200) {
          ...
          //     });
          // }
          // 下游用户等级折扣 price_difference_client_level_discount
          this.payDiscount = Number(res.data.data.price_difference_client_level_discount);
          // 开启了优惠码插件
          if (this.isShowPromo) {
            // 更新优惠码
          ...
          const calculatedPrice = (p * 1000 - this.payCodePrice * 1000 - this.payDiscount * 1000) / 1000;
      ...
      upgradePackagePrice(type, params)
        .then(async (res) => {
          if (res.data.status == 200) {
            ...
            //     });
            // }
            this.upParams.clDiscount = Number(res.data.data.price_difference_client_level_discount);
            // 开启了优惠码插件
            if (this.isShowPromo) {
              // 更新优惠码
              await applyPromoCode({
    ...
    getIpPrice() {
      ...
      ipPrice({
        ...
        .then(async (res) => {
          ...
          //     });
          // }
          this.ipDiscountkDisPrice = res.data.data.price_difference_client_level_discount * 1;
          // 开启了优惠码插件
          if (this.isShowPromo) {
            // 更新优惠码
            await applyPromoCode({
      ...
      this.orderTimer = setTimeout(() => {
        ...
        expanPrice(params)
          .then(async (res) => {
            ...
            //     });
            // }
            this.expansionDiscount = Number(ress.data.data.price_difference_client_level_discount);
            // 开启了优惠码插件
            if (this.isShowPromo) {
              // 更新优惠码
              await applyPromoCode({
    ...
    async getBsPrice(bol = false) {
      try {
        ...
        //     });
        // }
        this.bsData.moneyDiscount = Number(res.data.data.price_difference_client_level_discount);
        // 开启了优惠码插件
        if (this.isShowPromo) {
          // 更新优惠码
          await applyPromoCode({
