...
const app2 = Vue.createApp({
  ...
  computed: {
    ...
    calcIPList() {
      // 根据数据中心、机型、带宽、流量、操作系统来计算可选的 IP 列表
      ...
      if (temp.length === 0) {
        // 没有匹配的限制规则,返回原始数据
        // 对于 slider 类型,需要更新 ipv4Tip
        if (
          this.lineDetail.ip[0]?.type !== "radio" &&
          this.ipv4Arr &&
          this.ipv4Arr.length > 0
        ) {
      ...
      if (isRadio) {
        ...
        if (!validValues.includes(this.params.ip_num)) {
          this.params.ip_num = fArr[0]?.value || "";
          this.ipName = this.calcIpUnit(this.params.ip_num);
        }
      } else {
        // slider 类型：计算范围,返回对象数组（详情页特殊处理）
        // 确保 ipv4Arr 已初始化
        if (!this.ipv4Arr || this.ipv4Arr.length === 0) {
          return this.lineDetail.ip;
        }
        ...
        let finalRange = Array.from(new Set(ipOpt)).sort((a, b) => a - b);
        // 如果过滤后为空，使用原始数组
        if (finalRange.length === 0) {
          finalRange = this.ipv4Arr;
        }
        this.ipv4Tip = this.createTip(finalRange);

        // slider 类型返回对象数组，每个对象包含计算后的范围
        const minValue = finalRange[0];
        const maxValue = finalRange[finalRange.length - 1];
        fArr = this.lineDetail.ip.map((item) => {
          return {
            ...item,
            min_value: minValue,
            max_value: maxValue,
        ...
        // 如果当前值不在范围内，自动选择第一个
        if (!finalRange.includes(this.params.ip_num * 1)) {
          this.params.ip_num = minValue || "";
        }
      }
      return fArr;
    },
    ...
    calcIpUnit() {
      return (val) => {
        if (val === "NC") {
          // 把 this.ipData 改成 this.lineDetail.ip
          const isNc = this.lineDetail.ip.filter((item) => item.value === "NC");
          return isNc[0]?.desc || "NC"; // 加上 ?. 和默认值
        }
        let temp = "";
    ...
    return {
      ...
      powerStatusArr: {
        on: {
          ...
          icon: `/plugins/reserver/mf_dcim/template/clientarea/mobile/mfm301/img/mf_dcim/on.svg`,
        },
        off: {
          text: lang.common_cloud_text11,
          icon: `/plugins/reserver/mf_dcim/template/clientarea/mobile/mfm301/img/mf_dcim/off.svg`,
        },
        operating: {
          text: lang.common_cloud_text12,
          icon: `/plugins/reserver/mf_dcim/template/clientarea/mobile/mfm301/img/mf_dcim/operating.svg`,
        },
        fault: {
          text: lang.common_cloud_text86,
          icon: `/plugins/reserver/mf_dcim/template/clientarea/mobile/mfm301/img/mf_dcim/fault.svg`,
        },
        suspend: {
          text: lang.common_cloud_text87,
          icon: `/plugins/reserver/mf_dcim/template/clientarea/mobile/mfm301/img/mf_dcim/suspended.svg`,
        },
      },
      addons_js_arr: [], // 插件列表
  ...
  methods: {
    ...
    handleIpv4Range(ipv4Rule) {
      // 处理 ipv4_num 范围判断
      // 如果 ip_num 还未初始化，返回 true（不过滤）
      if (!this.params.ip_num) {
        return true;
      }
      const rawValue = String(this.params.ip_num).split("_")[0];
      const target = rawValue * 1;
    ...
    getManualOs() {
      manualResourceOs(this.id).then((res) => {
        if (res.data.status === 200) {
          ...
          this.osIcon =
            "/plugins/reserver/mf_dcim/template/clientarea/mobile/mfm301/img/mf_dcim/" +
            this.osData[0].name +
            ".svg";
    ...
    // #region 2026-3-24 续费弹窗逻辑已提取为公共组件 renewDialog，下面的暂时注释，后续可删除
    // 续费使用优惠码
    ...
    //   this.renewLoading = false;
    // },
    // #endregion 2026-3-24 续费弹窗逻辑已提取为公共组件 renewDialog，下面的暂时注释，后续可删除
      ...
      upgradePackagePrice(this.id, params)
        .then(async (res) => {
          if (res.data.status == 200) {
            ...
            //     });
            // }
            this.upParams.clDiscount = res.data.data.price_difference_client_level_discount * 1;
            // 开启了优惠码插件
            if (this.isShowPromo) {
              // 更新优惠码
              await applyPromoCode({
