...
const app2 = Vue.createApp({
  ...
  computed: {
    ...
    calcIPList() {
      // 根据数据中心、机型、带宽、流量、操作系统来计算可选的 IP 范围
      ...
      if (isRadio) {
          ...
          if (cur.result.ipv4_num[0].opt === "eq") {
            ...
            cur.result.ipv4_num.forEach((m) => {
              ...
              this.lineDetail.ip.forEach((item) => {
                const val = String(item.value).split("_")[0];
                const valNum = val === "NC" ? "NC" : val * 1;
                if (valNum !== "NC" && valNum >= min && valNum <= max) {
        ...
        // 如果当前值不在范围内，自动选择第一个
        if (!fArr.includes(this.params.ip_num * 1)) {
          this.params.ip_num = fArr[0] || "";
        }
        // slider 类型需要返回对象数组以匹配模板
        // 将数字数组转换为对象数组格式
        return fArr.map((num, index) => ({
          value: num,
          min_value: fArr[0],
          max_value: fArr[fArr.length - 1],
          desc: num + lang.mf_one,
          num: num,
        }));
      }
      return fArr;
