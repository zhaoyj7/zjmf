...
const app2 = Vue.createApp({
  ...
  computed: {
    calcCycleList() {
      return this.cycleList.map(item => {
        if (item.id === 'ontrial' && item.num && item.unit) {
          return {
            ...item,
            name: item.name + '(' + item.num + this.durationUnit[item.unit] + ')'
          };
        }
        return item;
      });
    ...
    calcIpv4Range() {
      ...
      // 如果没有限制规则,直接返回原始数据
      if (this.configLimitList.length === 0) {
        // radio 类型返回对象数组,slider 类型返回数字数组
        if (isRadio) {
          return this.lineDetail.ip;
        } else {
          // slider 类型:确保 ipv4Arr 已初始化
