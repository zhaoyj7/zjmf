...
class IdcsmartCommonProductController extends BaseController
{
     ...
     * @return  string configoptions[].option_type -  配置项类型：select下拉单选，multi_select下拉多选，radio点击单选，quantity数量输入，quantity_range数量拖动，yes_no是否，area区域，cascade级联
     ...
     * @return array configoptions[].tree - 级联配置项的树形结构数据(仅当option_type为cascade时返回)
     * @return int configoptions[].tree[].item_id - 级联项ID
     * @return string configoptions[].tree[].item_name - 级联项名称
     * @return string configoptions[].tree[].fee_type - 计费类型
     * @return int configoptions[].tree[].group_id - 级联组ID
     * @return int configoptions[].tree[].is_leaf - 是否为末端项:1是，0否
     * @return int configoptions[].tree[].order - 排序
     * @return int configoptions[].tree[].hidden - 是否隐藏:1是，0否
     * @return array configoptions[].tree[].children - 子级联项(递归结构)
     * @return array configoptions[].tree[].price - 价格信息(仅末端项有此字段)
     * @return array configoptions[].cascade_group - 级联组信息(仅当option_type为cascade时返回)
     * @return int configoptions[].cascade_group[].id - 级联组ID
     * @return string configoptions[].cascade_group[].group_name - 级联组名称
     ...
     * @param   object cascade_configoption - 级联配置项信息{169:{item_id:12,quantity:2}} optional
     * @return object cycles - 周期({"onetime":1.00})
     * @return array custom_cycles - 自定义周期
     * @return int custom_cycles[].id - 自定义周期ID
     * @return string custom_cycles[].name - 自定义周期名称
     ...
     * @return int configoptions[].cascade_group[].id - 级联组ID
     * @return string configoptions[].cascade_group[].group_name - 级联组名称
     * @return int configoptions[].current_cascade_item_id - 当前选择的级联项ID(仅当option_type为cascade时返回)
     ...
     * @version v1
     * @param   int host_id - 产品ID require
     * @param  object  - 与购物车计算价格参数一致:{"configoption":{"1"：2,"2":3,"4":[1,2,3]},"cascade_configoption":{"169":{"item_id":12,"quantity":2}},"cycle":"monthly","product_id":104,son:{}}
     */
     ...
     * @param   int host_id - 产品ID require
     * @param   int product_id - 商品ID require
     * @param   object config_options - 与购物车结算的一样:{"configoption":{"1"：2,"2":3,"4":[1,2,3]},"cascade_configoption":{"169":{"item_id":12,"quantity":2}},"cycle":"monthly","product_id":104,son:{}} require
     */
     ...
     * @return  string upgrade_configoptions[].option_type - 配置项类型：select下拉单选，multi_select下拉多选，radio点击单选，quantity数量输入，quantity_range数量拖动，yes_no是否，area区域，cascade级联
     * @return  int upgrade_configoptions[].option_name - 配置项名称
     * @return  array upgrade_configoptions[].subs - 配置子项数据
     * @return  array upgrade_configoptions[].tree - 级联配置项的树形结构数据(仅当option_type为cascade时返回)
     * @return  int upgrade_configoptions[].tree[].item_id - 级联项ID
     * @return  string upgrade_configoptions[].tree[].item_name - 级联项名称
     * @return  string upgrade_configoptions[].tree[].fee_type - 计费类型
     * @return  int upgrade_configoptions[].tree[].group_id - 级联组ID
     * @return  int upgrade_configoptions[].tree[].is_leaf - 是否为末端项:1是，0否
     * @return  int upgrade_configoptions[].tree[].order - 排序
     * @return  int upgrade_configoptions[].tree[].hidden - 是否隐藏:1是，0否
     * @return  array upgrade_configoptions[].tree[].children - 子级联项(递归结构)
     * @return  array upgrade_configoptions[].tree[].price - 价格信息(仅末端项有此字段)
     * @return  array upgrade_configoptions[].cascade_group - 级联组信息(仅当option_type为cascade时返回)
     * @return  int upgrade_configoptions[].cascade_group[].id - 级联组ID
     * @return  string upgrade_configoptions[].cascade_group[].group_name - 级联组名称
     */
     ...
     * @param   int host_id - 产品ID require
     * @param  object configoption - "configoption":{"1"：2,"2":3,"4":[1,2,3]}
     * @param  object cascade_configoption - 级联配置项参数 "cascade_configoption":{"1":{"item_id":123,"quantity":2}}
     * @return int id - 订单ID
     */
