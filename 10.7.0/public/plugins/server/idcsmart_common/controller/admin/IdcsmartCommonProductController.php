...
class IdcsmartCommonProductController extends BaseController
{
     ...
     * @title 更新周期状态
     * @desc 更新周期状态（启用/禁用）
     * @url /admin/v1/idcsmart_common/product/:product_id/cycle/:id/status
     * @method  PUT
     * @author hh
     * @version v1
     * @param   int product_id - 商品ID require
     * @param   int id - 周期ID require
     * @param   int status - 状态(0=禁用,1=启用) require
     * @return  int status - 状态码(200=成功,400=失败)
     * @return  string msg - 提示信息
     */
    public function updateCycleStatus()
    {
        $param = $this->request->param();

        if (!isset($param['status']) || !in_array($param['status'], [0, 1])) {
            return json(['status' => 400, 'msg' => '状态参数错误']);
        }

        $IdcsmartCommonCustomCycleModel = new \server\idcsmart_common\model\IdcsmartCommonCustomCycleModel();

        $result = $IdcsmartCommonCustomCycleModel->updateCycleStatus($param['id'], $param['status']);
