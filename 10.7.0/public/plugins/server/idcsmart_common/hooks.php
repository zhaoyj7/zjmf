...
add_hook('after_product_copy', function($param){
    try{
        ...
        if(!empty($idcsmartCommonProduct)){
            ...
            $IdcsmartCommonCascadeGroup = $IdcsmartCommonCascadeGroupModel->whereIn('configoption_id', array_keys($idcsmartCommonProductConfigoptionIdArr))->select()->toArray();
            foreach ($IdcsmartCommonCascadeGroup as $key => $value) {
                $id = $value['id'];
                $idcsmartCommonCascadeGroupIdArr[$id] = 0;
                unset($value['id']);
                $value['configoption_id'] = $idcsmartCommonProductConfigoptionIdArr[$value['configoption_id']];
                $r = $IdcsmartCommonCascadeGroupModel->create($value);
                $idcsmartCommonCascadeGroupIdArr[$id] = $r->id;
            }

            $IdcsmartCommonCascadeItemModel = new \server\idcsmart_common\model\IdcsmartCommonCascadeItemModel();
            $IdcsmartCommonCascadeItem = $IdcsmartCommonCascadeItemModel->whereIn('cascade_group_id', array_keys($idcsmartCommonCascadeGroupIdArr))->where('parent_item_id', 0)->select()->toArray();
            foreach ($IdcsmartCommonCascadeItem as $key => $value) {
                $id = $value['id'];
                $idcsmartCommonCascadeItemIdArr[$id] = 0;
                unset($value['id']);
                $value['cascade_group_id'] = $idcsmartCommonCascadeGroupIdArr[$value['cascade_group_id']];
                $r = $IdcsmartCommonCascadeItemModel->create($value);
                $idcsmartCommonCascadeItemIdArr[$id] = $r->id;
            }
            ...
            foreach ($IdcsmartCommonCascadeItem as $key => $value) {
                ...
                unset($value['id']);
                $value['cascade_group_id'] = $idcsmartCommonCascadeGroupIdArr[$value['cascade_group_id']];
                $value['parent_item_id'] = $idcsmartCommonCascadeItemIdArr[$value['parent_item_id']];
                $r = $IdcsmartCommonCascadeItemModel->create($value);
                $idcsmartCommonCascadeItemIdArr[$id] = $r->id;
            }
            ...
            foreach ($idcsmartCommonProductConfigoptionSub as $key => $value) {
                ...
                $value['cascade_item_id'] = $idcsmartCommonCascadeItemIdArr[$value['cascade_item_id']] ?? 0;
                ...
                foreach ($configoptionOrderItems as $configoptionOrderItem){
                    ...
                    foreach ($olds as $key=>$old){ // 考虑到配置项只有两个，且只有数量
                        ...
                        if ($configoption['option_type'] == 'cascade') {
                            // 级联配置项
                            $oldCascadeItem = $IdcsmartCommonCascadeItemModel->find($old['cascade_item_id']);
                            $newCascadeData = $new['cascade_configoption'][$old['configoption_id']] ?? [];
                            $newCascadeItemId = is_array($newCascadeData) ? ($newCascadeData['item_id'] ?? 0) : 0;
                            $newCascadeItem = $IdcsmartCommonCascadeItemModel->find($newCascadeItemId);

                            $oldCascadeName = $oldCascadeItem ? $IdcsmartCommonCascadeItemModel->getFullName($oldCascadeItem['id']) : '';
                            $newCascadeName = $newCascadeItem ? $IdcsmartCommonCascadeItemModel->getFullName($newCascadeItem['id']) : '';

                            // 处理数量类型的级联配置项
                            $oldFeeType = $oldCascadeItem['fee_type'] ?? '';
                            $newFeeType = $newCascadeItem['fee_type'] ?? '';
                            
                            if (in_array($oldFeeType, ['qty', 'stage']) && !empty($old['qty'])) {
                                $oldCascadeName .= ' x' . $old['qty'];
                            }
                            
                            $newQuantity = is_array($newCascadeData) ? ($newCascadeData['quantity'] ?? 1) : 1;
                            if (in_array($newFeeType, ['qty', 'stage']) && !empty($newQuantity)) {
                                $newCascadeName .= ' x' . $newQuantity;
                            }

                            $description[] = $configoption['option_name'] . ':' . $oldCascadeName . '=>' . $newCascadeName;
                        } elseif ($IdcsmartCommonLogic->checkQuantity($configoption['option_type'])){
            ...
            elseif($upgrade['type']=='product'){ // 产品升降级描述
                ...
                if (!empty($productOrderItems)){
                    foreach ($productOrderItems as $productOrderItem){
                        ...
                        // 处理级联配置项
                        if (!empty($new['cascade_configoption'])){
                            foreach ($new['cascade_configoption'] as $k=>$cascadeData){
                                $productUpgradeConfigoption = $IdcsmartCommonProductConfigoptionModel->find($k);
                                if ($productUpgradeConfigoption && $productUpgradeConfigoption['option_type'] == 'cascade'){
                                    $cascadeItemId = is_array($cascadeData) ? ($cascadeData['item_id'] ?? 0) : 0;
                                    $cascadeItem = $IdcsmartCommonCascadeItemModel->find($cascadeItemId);
                                    if ($cascadeItem) {
                                        $cascadeName = $IdcsmartCommonCascadeItemModel->getFullName($cascadeItem['id']);
                                        
                                        // 处理数量类型的级联配置项
                                        $feeType = $cascadeItem['fee_type'] ?? '';
                                        $quantity = is_array($cascadeData) ? ($cascadeData['quantity'] ?? 1) : 1;
                                        if (in_array($feeType, ['qty', 'stage']) && !empty($quantity)) {
                                            $cascadeName .= ' x' . $quantity;
                                        }
                                        
                                        $subDescription .= $productUpgradeConfigoption['option_name'] . ":" . $cascadeName . ';';
                                    }
                                }
                            }
                        }
...
// 商品自然月预付费开关变更钩子
add_hook('product_natural_month_prepaid_change', function($param){
    ...
    if ($status == 1) {
        // 开启自然月预付费
        return $IdcsmartCommonCustomCycleModel->enableNaturalMonthCycles($productId);
    } else {
        // 关闭自然月预付费
        return $IdcsmartCommonCustomCycleModel->disableNaturalMonthCycles($productId);
    }
});