...
class IdcsmartCommonProductConfigoptionModel extends Model
{
    ...
    public function createConfigoption($param)
    {
        ...
        try{
            ...
            // 如果是级联配置项，自动创建默认级联组和级联项
            if ($param['option_type'] == 'cascade'){
                $IdcsmartCommonCascadeLogic = new \server\idcsmart_common\logic\IdcsmartCommonCascadeLogic();
                $initResult = $IdcsmartCommonCascadeLogic->initializeCascadeConfigoption($id);
                if ($initResult['status'] != 200) {
                    throw new \Exception($initResult['msg']);
                }
            }
    ...
    public function deleteConfigoption($param)
    {
        ...
        try{
            ...
            # 如果是级联配置项，级联删除所有级联组、级联项、配置子项和价格
            if ($configoption['option_type'] == 'cascade') {
                ...
                $groups = $IdcsmartCommonCascadeGroupModel->where('configoption_id', $id)->select();
                foreach ($groups as $group) {
                    $IdcsmartCommonCascadeGroupModel->deleteCascadeGroup(['id' => $group['id']]);
                }
            }
