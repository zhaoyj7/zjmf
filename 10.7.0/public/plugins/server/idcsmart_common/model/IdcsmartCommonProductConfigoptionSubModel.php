...
class IdcsmartCommonProductConfigoptionSubModel extends Model
{
    ...
    public function indexConfigoptionSub($param)
    {
        ...
        $customCycles = $IdcsmartCommonCustomCycleModel
            ->field('id,name')
            ->where('product_id',$productId)
            ->where('cycle_type', $product['natural_month_prepaid'])
            ->select()
            ->toArray();
        # 配置子项的自定义周期及价格
    ...
    public function createConfigoptionSub($param)
    {
        ...
        try{
            ...
            $subId = $this->insertGetId([
                ...
                'cascade_item_id' => $param['cascade_item_id'] ?? 0,
    ...
    public function updateConfigoptionSub($param)
    {
        ...
        try{
            ...
            $configoptionSub->save([
                'product_configoption_id' => $configoptionId,
                'cascade_item_id' => $param['cascade_item_id'] ?? $configoptionSub['cascade_item_id'],
                'option_name' => $param['option_name']??'',
                'option_param' => $param['option_param']??'',
                'qty_min' => $param['qty_min']??0,
                'qty_max' => $param['qty_max']??0,
    ...
    /**
     * 时间 2024-12-20
     * @title 通过级联项ID获取配置子项列表
     * @desc 获取指定级联项的所有配置子项
     * @author theworld
     * @version v1
     * @param int cascade_item_id - 级联项ID require
     * @return array list - 配置子项列表
     */
    public function getSubsByCascadeItemId($cascadeItemId)
    {
        $list = $this->where('cascade_item_id', $cascadeItemId)
            ->order('order', 'asc')
            ->order('id', 'asc')
            ->select()
            ->toArray();
    ...
    /**
     * 时间 2024-12-20
     * @title 通过级联项ID获取配置子项（带价格）
     * @desc 获取指定级联项的所有配置子项及其价格信息
     * @author theworld
     * @version v1
     * @param int cascade_item_id - 级联项ID require
     * @return array list - 配置子项列表（包含价格）
     */
    public function getSubsWithPriceByCascadeItemId($cascadeItemId)
    {
        $subs = $this->alias('cs')
            ->field('cs.*,p.onetime')
            ->leftJoin('module_idcsmart_common_pricing p', 'p.rel_id=cs.id AND p.type=\'configoption\'')
            ->where('cs.cascade_item_id', $cascadeItemId)
            ->order('cs.order', 'asc')
            ->order('cs.id', 'asc')
            ->select()
            ->toArray();
        ...
        $IdcsmartCommonCustomCyclePricingModel = new IdcsmartCommonCustomCyclePricingModel();
        foreach ($subs as &$sub) {
            $customCyclePrices = $IdcsmartCommonCustomCyclePricingModel
                ->alias('ccp')
                ->field('ccp.custom_cycle_id,ccp.amount,cc.name')
                ->leftJoin('module_idcsmart_common_custom_cycle cc', 'cc.id=ccp.custom_cycle_id')
                ->where('ccp.rel_id', $sub['id'])
                ->where('ccp.type', 'configoption')
                ->select()
                ->toArray();
            
            $sub['custom_cycle'] = $customCyclePrices;
        }
     ...
     * 时间 2024-12-20
     * @title 删除级联项的所有配置子项
     * @desc 删除指定级联项的所有配置子项及其价格
     * @author theworld
     * @version v1
     * @param int cascade_item_id - 级联项ID require
     * @return bool
     */
    public function deleteSubsByCascadeItemId($cascadeItemId)
    {
        $this->startTrans();

        try {
            $subs = $this->where('cascade_item_id', $cascadeItemId)->select();
