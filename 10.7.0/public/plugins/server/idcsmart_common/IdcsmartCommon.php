...
class IdcsmartCommon
{
    ...
    {
        return ['display_name'=>'通用产品', 'version'=>'4.0.0'];
    }
  ...
  `cycle_type` tinyint(1) NOT NULL DEFAULT '0' COMMENT '周期类型(0=普通周期,1=自然月周期)',
  ...
  `cascade_item_id` int(11) NOT NULL DEFAULT '0' COMMENT '级联项ID',
  PRIMARY KEY (`id`),
  KEY `host_id` (`host_id`),
  KEY `cascade_item_id` (`cascade_item_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4;",
  ...
  `cascade_item_id` int(11) NOT NULL DEFAULT '0' COMMENT '级联项ID，用于级联配置项',
  ...
  UNIQUE KEY `host_id` (`host_id`)
            ...
            "CREATE TABLE `idcsmart_module_idcsmart_common_cascade_group` (
  ...
  KEY `level` (`level`)
            ...
            "CREATE TABLE `idcsmart_module_idcsmart_common_cascade_item` (
  ...
  KEY `is_leaf` (`is_leaf`)
    ...
    public function afterDeleteLastServer()
    {
        $sql = [
            ...
            "DROP TABLE IF EXISTS `idcsmart_module_idcsmart_common_cascade_group`;",
            "DROP TABLE IF EXISTS `idcsmart_module_idcsmart_common_cascade_item`;",
        ];
...
//        return $res;
    ...
    public function changePackage($params){
        ...
        $cascadeData = $params['custom']['cascade_configoption']??[];
        ...
        try{
            ...
            foreach ($oldConfigoptions as $oldConfigoption){
                ...
                if (!empty($configoption)){
                    ...
                    }elseif ($configoption['option_type'] == 'cascade'){
                        // 级联配置项处理:直接使用cascade_item_id字段
                        if (!empty($oldConfigoption['cascade_item_id'])){
                            $packageData['cascade_configoptions'][$configoption['option_param']] = [
                                'item_id' => $oldConfigoption['cascade_item_id'],
                                'quantity' => $oldConfigoption['qty'] > 0 ? $oldConfigoption['qty'] : 1
                            ];
                        }
                    }else{
            ...
            if (!empty($cascadeData)) {
                ...
                foreach ($cascadeData as $configoptionId => $cascadeItem) {
                    // 验证配置项是否为级联类型
                    ...
                    $itemId = $cascadeItem['item_id'] ?? 0;
                    $quantity = $cascadeItem['quantity'] ?? 1;
                    ...
                    // 验证级联项是否存在且为末端项
                    $cascadeItemInfo = $IdcsmartCommonCascadeItemModel->find($itemId);
                    if (empty($cascadeItemInfo) || $cascadeItemInfo['is_leaf'] != 1) {
                        continue;
                    }
            ...
            if ($upstreamProduct){ // 同步代理模式
                ...
                if ($result['status']==200){
                    ...
                    if ($result['status']==200 || ($result['status']==400 && !empty($result['data']['no_credit']))){
                        // 使用平台币支付
                        if (!empty($result['data']['customfields']['agent_use_coin'])){
                            $resultCoin = idcsmart_api_curl($upstreamProduct['supplier_id'],'/console/v1/coin/pay',[
                                'auto'=>1,
                                'coin_coupon_ids'=>[],
                        ...
                        // 亏本交易拦截检查
                        $checkResult = checkLossTrade($orderId, $result['data']['amount']??0, 'upgrade', '', $params['host']['id']);
                        if (!$checkResult['pass']) {
                        ...
                        // 余额支付失败，尝试信用额支付
                        if ($result['status'] != 200) {
                            idcsmart_api_curl($upstreamProduct['supplier_id'],'/console/v1/credit',['id'=>$payData['id'],'use'=>0]);
                            $creditLimitResult = idcsmart_api_curl($upstreamProduct['supplier_id'],'/console/v1/credit_limit/pay',['id'=>$payData['id']]);
    ...
    public function changeProduct($params)
    {
        ...
        // 获取旧配置
        $productId = $params['custom']['old_product_id']??0;
        $IdcsmartCommonProductModel = new IdcsmartCommonProductModel();
        $commonProduct = $IdcsmartCommonProductModel->where('product_id',$productId)->find();
        $IdcsmartCommonHostConfigoptionModel = new IdcsmartCommonHostConfigoptionModel();
        $hostConfigoptions = $IdcsmartCommonHostConfigoptionModel->where('host_id',$params['host']['id'])
            ->select()
            ->toArray();

        $oldProduct = [
            'common_product' => $commonProduct,
            'host_configoptions' => $hostConfigoptions
        ];
        ...
        # 配置中关联子模块,调子模块中开通,使用以前的逻辑
        $ProvisionLogic = new ProvisionLogic();
        $res = $ProvisionLogic->changeProduct($params['host']['id'],$oldProduct);
        return $res;
    }
...
-----END PUBLIC KEY-----";
    ...
    public function adminField($params)
    {
        $ProvisionLogic = new ProvisionLogic();
        $res = $ProvisionLogic->adminField($params['host']['id']);
        return $res;
    }

    public function hostUpdate($params)
    {
        $ProvisionLogic = new ProvisionLogic();
        $res = $ProvisionLogic->hostUpdate($params['host']['id']);
        if (!empty($res['no_support_function'])){
            return ['status'=>200];
        }
        return $res;
    }
