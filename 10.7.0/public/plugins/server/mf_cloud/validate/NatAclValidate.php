...
class NatAclValidate extends Validate
{
    protected $rule = [
        ...
        'ext_port'  => 'checkExtPort:thinkphp',
    ];
    ...
    protected $message = [
        ...
        'ext_port.checkExtPort' => 'mf_cloud_ext_port_format_error',
    ];

   	protected $scene = [
   		'create' => ['name','int_port','protocol','ext_port'],
   	];

    // 验证外部端口
    public function checkExtPort($value){
        if(empty($value)){
            return true;
        }
        return is_numeric($value) && $value >= 1 && $value <= 65535 && $value != 80 && $value != 443 && $value != 22;
    }
