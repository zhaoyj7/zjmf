...
class CartValidate extends Validate
{
	protected $rule = [
        ...
        'snap_num'                  => 'integer',
        'duration_id'               => 'require',
        'password'                  => 'checkPassword:thinkphp',
        'ssh_key_id'                => 'integer|checkSSHKeyId:thinkphp',
        'notes'                     => 'length:0,1000',
    ...
    protected $message  =   [
        ...
        'rand_password.in'                      => 'param_error',
        'rand_port.in'                          => 'param_error',
    ];

    protected $scene = [
        // 下单验证
        'cal'               => ['data_center_id','cpu','memory','image_id','system_disk','data_disk','backup_num','snap_num','duration_id','password','ssh_key_id','notes','network_type','bw','recommend_config_id','resource_package_id','flow','ip_num','gpu_num','port','ipv6_num','rand_password','rand_port'],
    ...
    public function checkPassword($value, $rule, $data){
        if(empty($data['ssh_key_id']) && empty($data['rand_password']) && empty($value)){
            return 'please_set_login_password';
        }
        return ToolLogic::checkPassword($value);
    }

    // 验证密钥
    public function checkSSHKeyId($value, $rule, $data){
        if(empty($data['rand_password']) && empty($data['password']) && empty($value)){
            return 'password_and_ssh_key_must_have_one';
        }
        return true;
    }
    ...
    public function checkSecurityGroupProtocol($value){
        $allow = ['icmp','ssh','telnet','http','https','mssql','oracle','mysql','rdp','postgresql','redis','remote_port'];
        foreach($value as $v){
            if(!in_array($v, $allow) && strpos($v, 'id_') !== 0){
