...
class CloudController
{
     ...
     * @return  string security_group_config[].id - 安全组标识(remote_port是原来远程端口)
     * @return  string security_group_config[].description - 描述
     * @return  string security_group_config[].port - 端口
     * @return  string security_group_config[].protocol - 协议
	 */
     ...
     * @param   int is_market - 是否镜像市场(0=普通镜像,1=镜像市场镜像)
     ...
     * @return  int list[].image[].is_market - 是否镜像市场(0=普通镜像,1=镜像市场镜像)
     * @return  int market_image_count - 镜像市场镜像数量
	 */
	...
	*/
     ...
     * @return  int config.manual_manage - 手动管理商品(0=关闭,1=开启)
     * @return  int config.simulate_physical_machine_enable - 模拟物理机运行(0=关闭,1=开启)
 ...
 //        $VpcNetworkModel = new VpcNetworkModel();
	...
	public function changeVpcNetwork()
	{
        ...
        try{
        	$CloudLogic = new CloudLogic($id);

			$result = $CloudLogic->changeVpcNetwork($param);
        }catch(\Exception $e){
			// 模块不对，可能是数据库
			if($e->getMessage() == lang_plugins('can_not_do_this') && class_exists('server\mf_cloud_mysql\logic\CloudLogic')){
				try{
					$CloudLogic = new \server\mf_cloud_mysql\logic\CloudLogic($id);
        ...
        if($param['custom']['network_type'] == 'vpc'){
        	if(isset($param['custom']['vpc'])){
        		if(isset($param['custom']['vpc']['id']) && $param['custom']['vpc']['id']>0){
        			$VpcNetworkModel = new VpcNetworkModel();
					if(!$VpcNetworkModel->isSupportVPC()){
						return json(['status'=>400, 'msg'=>lang_plugins('vpc_network_not_found') ]);
					}
        			$vpcNetwork = VpcNetworkModel::find($param['custom']['vpc']['id']);
        			if(empty($vpcNetwork) || $vpcNetwork['client_id'] != get_client_id()){
        				return json(['status'=>400, 'msg'=>lang_plugins('vpc_network_not_found')]);
        			}
					if(!$vpcNetwork->checkVpcIsEnable($vpcNetwork, $param['id'], $param['custom']['data_center_id'])){
						return json(['status'=>400, 'msg'=>lang_plugins('vpc_network_not_found')]);
					}
	 ...
	 * @author hh
	 * @version v1
	 * @url /console/v1/mf_cloud/:id/traffic_package
	 * @method  GET
	 * @param   int id - 产品ID require
	 * @param   int page - 页数
	 * @param   int limit - 每页条数
	 ...
	 * @author hh
	 * @version v1
	 * @url /console/v1/mf_cloud/:id/check_refund
	 * @method  GET
	 * @param   int id - 产品ID require
	 */
	public function checkRefund(): Json
	{
		$param = request()->param();
