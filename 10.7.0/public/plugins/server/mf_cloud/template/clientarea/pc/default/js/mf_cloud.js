...
new Vue({
  ...
  data() {
    return {
      ...
      imageType: "public", // public market
    };
  },
  methods: {
    changeSshPort() {
    ...
    handleRange(item, type) {
      ...
      // 内存 改为单选和范围
      if (item[type].value) {
        // value 是离散值数组，直接检查是否包含，而不是转换成范围
    ...
    async getConfig(bol = true, type) {
      try {
        ...
        this.diskStep =
          (this.baseConfig.disk_range_limit_switch && this.baseConfig.disk_range_limit) ?
    ...
    async getLineDetails(id) {
      try {
        ...
        } else if (this.lineDetail.bill_type === "flow") {
          this.params.bw = "";
        }
    ...
    changeSysNum(num) {
      ...
      // 如果对齐后的值不在允许范围内，找到最近的允许值
      if (!temp.includes(alignedNum)) {
        temp.forEach((item, index) => {
          if (alignedNum > item && alignedNum < temp[index + 1]) {
            let res = alignedNum - item > temp[index + 1] - alignedNum ? temp[index + 1] : item;
            this.$nextTick(() => {
