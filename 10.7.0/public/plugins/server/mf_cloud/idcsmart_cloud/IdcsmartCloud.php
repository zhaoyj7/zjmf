...
class IdcsmartCloud{
	 ...
	 * @param   int ipv6_group_id IPv6分组ID
	 */
	public function cloudModifyIpv6(int $id, $num = 0, $ipv6_group_id = 0){
		return $this->request('clouds/'.$id.'/ipv6', ['num'=>$num, 'ipv6_group_id'=>$ipv6_group_id], 'PUT');
	}
	 ...
	 * @param   int id - 实例ID require
	 * @param   array param - 参数 require
	 * @param   string param.name - 流量包名称
	 * @param   int param.size - 流量包大小 require
	 * @param   int param.expire_time - 到期时间(秒级时间戳,0表示不到期) require
	 * @param   int param.expire_with_reset 1 是否在流量重置时过期(0=否,1=是)
	 * @param   string param.rel_id - 外部关联ID
	 */
	public function createTrafficPackage($id, $param = [])
	{
		return $this->request('clouds/'. $id .'/traffic_package', $param, 'POST');
	}
	 ...
	 * @param   int id - 实例ID require
	 */
	public function trafficPackageList($id, $param = [])
	{
		return $this->request('clouds/'. $id .'/traffic_package', $param, 'GET');
	}
