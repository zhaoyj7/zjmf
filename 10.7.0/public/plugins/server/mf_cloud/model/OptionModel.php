...
class OptionModel extends Model
{
    ...
    public function optionIndex($id): array
    {
        ...
        // 获取商品自然月开关状态
        // $product = ProductModel::find($option['product_id']);
        // $cycleType = !empty($product) && $product['natural_month_prepaid'] == 1 ? 1 : 0;
    ...
    public function lineDefenceDragSort($param)
    {
        ...
        $productId = $lineDefence['product_id'];
        $relId = $lineDefence['rel_id'];

        // 开启事务
        $this->startTrans();
        try {
            // 获取当前产品线路下的所有防御配置，按 order asc, value asc 排序
            $allItems = $this->where('rel_type', 4)
                ->where('product_id', $productId)
                ->where('rel_id', $relId)
                ->order('order', 'asc')
                ->order('value', 'asc')
            ...
            // 从列表中移除当前拖动的项
            $currentIndex = array_search($param['id'], $allItems);
            if ($currentIndex !== false) {
                array_splice($allItems, $currentIndex, 1);
            }

            // 找到目标位置
            if ($param['prev_id'] == 0) {
                // 插入到最前面
                array_unshift($allItems, $param['id']);
            } else {
                $prevIndex = array_search($param['prev_id'], $allItems);
                if ($prevIndex === false) {
                    $this->rollback();
                    return ['status' => 400, 'msg' => lang_plugins('mf_cloud_option_not_found')];
                }
                // 插入到 prev_id 后面
                array_splice($allItems, $prevIndex + 1, 0, $param['id']);
            }

            // 重新分配 order 值
            $allItems = array_values($allItems);
            foreach ($allItems as $index => $id) {
                $this->where('id', $id)->update(['order' => $index]);
            }
    ...
    public function matchOption($productId, $relType, $relId = 0, $value = 0, $diskType = NULL)
    {
        ...
        if($type == 'radio'){
            ...
            if(!empty($option)){
                // 如果是磁盘，验证磁盘大小购买限制
                if($relType == OptionModel::DATA_DISK || $relType == OptionModel::SYSTEM_DISK){
                    static $diskRangeLimit;
                    if(empty($diskRangeLimit)){
                        $diskRangeLimit = 1;
                        $config = ConfigModel::field('disk_range_limit_switch,disk_range_limit')->where('product_id',$productId)->find();
                        if(!empty($config) && $config['disk_range_limit_switch'] == 1){
                            $diskRangeLimit = $config['disk_range_limit'];
                        }
                    }
                    if($value % $diskRangeLimit != 0){
                        $option = NULL;
                    }
                }
            }else{
