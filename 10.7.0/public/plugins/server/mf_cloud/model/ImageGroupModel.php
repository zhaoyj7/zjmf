...
class ImageGroupModel extends Model
{
    ...
    public function imageGroupCreate($param)
    {
        ...
        $description = lang_plugins('log_mf_cloud_add_image_group_success', [
    ...
    public function imageGroupUpdate($param)
    {
        ...
        if($imageGroup['name'] != $param['name']){
            ...
            $description = lang_plugins('log_mf_cloud_modify_image_group_success', [
                '{product}' => 'product#'.$imageGroup['product_id'].'#'.$productName.'#',
    ...
    public function imageGroupDelete($id)
    {
        ...
        $description = lang_plugins('log_mf_cloud_delete_image_group_success', [
            '{product}' => 'product#'.$imageGroup['product_id'].'#'.$productName.'#',
            '{name}'    => $imageGroup['name'],
        ]);
        active_log($description, 'product', $imageGroup['product_id']);
     ...
     * @author hh
     * @version v1
     * @param   string param.icon - 图标 require
     * @return  bool
     */
    public function isWindows($param)
    {
        return isset($param['icon']) && strtolower($param['icon']) === 'windows';
    }
