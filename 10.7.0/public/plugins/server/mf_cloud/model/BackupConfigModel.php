...
class BackupConfigModel extends Model
{
     ...
     * @return  float list[].on_demand_price - 按需价格
     * @return  int count - 总条数
     */
    public function backupConfigList($param): array
    {
        $data = [];
        $count = 0;
        ...
        if(isset($param['product_id']) && !empty($param['product_id'])){
            ...
            $data = $this
                    ->field('id,num,price,on_demand_price')
                    ->where($where)
                    ->order('num', 'asc')
                    ->select()
                    ->toArray();
     ...
     * @param   float data[].price - 价格
     * @param   float data[].on_demand_price - 按需价格
    ...
    public function saveBackupConfig($product_id, $data, $type = 'backup')
    {
        ...
        foreach($data as $v){
            $backup_data[] = [
                ...
                'on_demand_price' => $v['on_demand_price'] ?? '0.0000',
            ];
        }
     ...
     * @return  BackupConfigModel|null
     */
    public function matchBackupConfig($param)
    {
        $backupConfig = $this
                    ->where('product_id', $param['product_id'])
                    ->where('type', $param['type'])
