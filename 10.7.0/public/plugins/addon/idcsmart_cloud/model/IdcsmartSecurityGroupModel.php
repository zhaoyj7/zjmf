...
class IdcsmartSecurityGroupModel extends Model
{
     ...
     * @return int data.id - 安全组ID
     */
    public function createIdcsmartSecurityGroup($param): array
    {
        $param['auto_create_rule'] = $param['auto_create_rule'] ?? 1;
        ...
        return ['status' => 200, 'msg' => lang_plugins('create_success'), 'data'=>['id'=>(int)$idcsmartSecurityGroup->id ]];
    }
     ...
     * @return string msg - 提示信息
     */
    public function updateIdcsmartSecurityGroup($param): array
    {
     ...
     * @return string msg - 提示信息
     */
    public function deleteIdcsmartSecurityGroup($id): array
    {
        // 验证安全组ID
        ...
        $count = IdcsmartSecurityGroupHostLinkModel::alias('aisghl')
            ->join('host h', 'h.id=aisghl.host_id')
            ->where('aisghl.addon_idcsmart_security_group_id', $id)
            ->where('h.is_delete', 0)
            ->where('h.status', 'Active')
            ->count();
        if($count > 0){
            return ['status'=> 400, 'msg'=> lang_plugins('addon_idcsmart_security_group_already_link_cannot_delete')];
        }
