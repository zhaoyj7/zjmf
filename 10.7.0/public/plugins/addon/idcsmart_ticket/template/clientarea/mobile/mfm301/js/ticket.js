    ...
    const app = Vue.createApp({
      ...
      methods: {
        ...
        selectTicketType(value) {
          this.params.ticket_type_id = value;
        },
        selectStatus(value) {
          if (this.params.status.includes(value)) {
            this.params.status.splice(this.params.status.indexOf(value), 1);
          } else {
            this.params.status.push(value);
          }
        },
        handleCancel() {
          this.params = JSON.parse(JSON.stringify(this.originParams));
        ...
        calcProductName(item) {
          ...
          if (item.due_time > 0) {
            if (item.due_time <= curTime) {
              ...
              // 即将到期
              return `(${item.name.slice(0, 8)}......${item.name.slice(-5)})${lang.ticket_label22
                }`;
            } else {
              return `(${item.name})`;
            }
