...
class TicketController extends PluginAdminBaseController
{
     ...
     * @return int ticket.replies[].quote_reply_id - 引用的回复ID,0表示未引用
     * @return object ticket.replies[].quote_info - 引用的回复信息,未引用时为null
     * @return int ticket.replies[].quote_info.id - 被引用回复ID
     * @return string ticket.replies[].quote_info.content - 被引用内容(截取前100字)
     * @return string ticket.replies[].quote_info.type - 类型:Client/Admin
     * @return string ticket.replies[].quote_info.sender_name - 发送者名称
     * @return int ticket.replies[].quote_info.create_time - 发送时间
     * @return int ticket.replies[].quote_info.is_deleted - 该回复是否已被删除:1是,0否
     ...
     * @param int quote_reply_id - 引用的回复ID,可选,默认0表示不引用
