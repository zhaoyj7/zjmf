...
class IdcsmartRefundProductModel extends Model
{
     ...
     * @return int list[].full_refund_days - 全额退款天数,0表示不启用
    ...
    public function refundProductList($param)
    {
        ...
        $refundProducts = $this->alias('rp')
            ...
            rp.require,rp.range_control,rp.range,rp.full_refund_days,rp.rule,rp.action,rp.api_refund_allow')
...
//        }
     ...
     * @param int full_refund_days - 全额退款天数,0表示不启用,默认为0
        ...
        try{
            foreach ($param['product_ids'] as $productId){
                ...
                $refundProduct = $this->create([
                    ...
                    'range' => $param['range']??0,
                    'full_refund_days' => $param['full_refund_days']??0,
                    'rule' => $param['rule']??'',
                    'ratio_value' => $param['rule']=='Ratio'?$param['ratio_value']:0,
                    'create_time' => time(),
                    'action' => $param['action']??'Suspend',
                    'api_refund_allow' => $param['api_refund_allow']??1,
                ]);
     ...
     * @return int full_refund_days - 全额退款天数,0表示不启用
    ...
    public function indexRefundProduct($id)
    {
        $refundProduct = $this->alias('rp')
            ->field('rp.id,rp.product_id,rp.type,rp.require,rp.range_control,rp.range,rp.full_refund_days,rp.rule,rp.ratio_value,p.pay_type,rp.action,rp.api_refund_allow')
            ->leftJoin('product p','p.id=rp.product_id')
            ->where('rp.id',$id)
            ->find();
