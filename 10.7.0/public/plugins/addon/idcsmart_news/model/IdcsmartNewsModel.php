...
class IdcsmartNewsModel extends Model
{
    ...
    public function idcsmartNewsList($param, $app = '')
    {
        ...
        // 修改排序规则：置顶新闻优先，然后按置顶时间倒序，最后按原有规则排序
        $orderRule = 'aih.is_top DESC, aih.top_time DESC, ' . $param['orderby'] . ' ' . $param['sort'];
        
        $list = $this->alias('aih')
            ->field('aih.id,aih.title,aih.img,aiht.name type,a.name admin,aih.create_time,aih.hidden,aih.cron_release,aih.cron_release_time,aih.is_top,aih.top_time')
        ...
        foreach ($list as $key => $value) {
            if($app=='home'){
                ...
                unset($list[$key]['admin'], $list[$key]['type'], $list[$key]['hidden'], $list[$key]['cron_release'], $list[$key]['cron_release_time'], $list[$key]['is_top'], $list[$key]['top_time']);
            }else if($app=='index'){
                // 计算发布时间
                if($value['cron_release'] == 1 && $value['cron_release_time'] <= $time){
                    $list[$key]['create_time'] = $value['cron_release_time'];
                }
                unset($list[$key]['admin'], $list[$key]['hidden'], $list[$key]['cron_release'], $list[$key]['cron_release_time'], $list[$key]['is_top'], $list[$key]['top_time']);
            }else{
...
//                    }
     ...
     * @version v1
     * @param int param.id - 新闻ID required
     * @param int param.is_top - 0取消置顶1置顶 required
     * @return int status - 状态码,200成功,400失败
     * @return string msg - 提示信息
     */
    public function topIdcsmartNews($param)
    {
        // 验证新闻ID
        $idcsmartNews = $this->find($param['id']);
        if(empty($idcsmartNews)){
        ...
        try {
            ...
            if($param['is_top'] == 1){
                $updateData['top_time'] = time();
            } else {
                $updateData['top_time'] = 0;
            }

            $this->update($updateData, ['id' => $param['id']]);

            # 记录日志
            if($param['is_top']==1){
                active_log(lang_plugins('log_admin_top_news', ['{admin}'=>request()->admin_name,'{title}'=>$idcsmartNews['title']]), 'addon_idcsmart_news', $idcsmartNews->id);
            }else{
                active_log(lang_plugins('log_admin_untop_news', ['{admin}'=>request()->admin_name,'{title}'=>$idcsmartNews['title']]), 'addon_idcsmart_news', $idcsmartNews->id);
            }
