...
class IdcsmartCertification extends Plugin
{
  ...
  KEY `client_id` (`client_id`)
        ...
        $applyTemplate = IdcsmartCertificationLogic::getDefaultConfig('certification_apply_notice_template');
    ...
    public function upgrade()
    {
        ...
        if(isset($plugin['version'])){
            ...
            if(version_compare('3.1.0', $plugin['version'], '>')){
				# 插入实名认证审核通知动作
				$applyTemplate = IdcsmartCertificationLogic::getDefaultConfig('certification_apply_notice_template');
                if(!empty($applyTemplate)){
                    foreach ($applyTemplate as $key=>$template){
            ...
            if(version_compare('3.1.4', $plugin['version'], '>')){
                # 新增企业认证需先完成个人认证配置
                if(!isset($config['certification_company_need_person'])){
                    $config['certification_company_need_person'] = 0;
                    $PluginModel->where('name','IdcsmartCertification')
                        ->where('module','addon')
                        ->update([
                            'config' => json_encode($config),
