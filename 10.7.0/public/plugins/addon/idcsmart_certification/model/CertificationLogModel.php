...
class CertificationLogModel extends Model
{
    ...
    public function certificationList($param)
    {
        ...
        $where = function (Query $query) use($param,$ids) {
            if(!empty($param['keywords'])){
                $query->where('cl.id|cl.card_name|cl.company|c.username|cl.certify_id|cl.card_number', 'like', "%{$param['keywords']}%");
            }
        ...
        $logs = $this->alias('cl')
            ->field('cl.id,cl.card_name,card_number,cl.company,cl.type,cl.status,cl.create_time,c.username,cl.client_id,cl.auth_fail,cl.company_organ_code,cl.certify_id')
    ...
    public function certificationIndex($param)
    {
        ...
        unset($log['plugin_name'],$log['custom_fields_json']);
    ...
    public function certificationInfo($param)
    {
        ...
        $configuration = [
            ...
            'certification_company_need_person' => $config['certification_company_need_person']??0,
        ];
    ...
    public function certificationCompany($param)
    {
        ...
        try{
            ...
            # 企业认证需先完成个人认证
            if (!$this->isAdmin && !empty($config['certification_company_need_person'])){
                $CertificationPersonModel = new CertificationPersonModel();
                $certificationPerson = $CertificationPersonModel->where('client_id',$clientId)
                    ->where('status',1)
                    ->find();
                if (empty($certificationPerson)){
                    throw new \Exception(lang_plugins('certification_company_need_person_first'));
                }
            }
        ...
        if ($status==1){ # 1通过
            ...
            if ($config['certification_approval'] || in_array($certificationLog['type'],[2,3])){
                ...
                system_notice([
                    ...
                    'email_description' => lang_plugins('idcsmart_certification_apply_notice_send_mail'),
                    'sms_description' => lang_plugins('idcsmart_certification_apply_notice_send_sms'),
                    'task_data' => [
                        'client_id' => $clientId,
                        'template_param' => [
                            'client_id' => $clientId,
                            'admin_url' => request()->domain() . '/admin/plugin/idcsmart_certification/index',
                        ],
                    ],
                ]);
