...
class IdcsmartWithdraw extends Plugin
{
  ...
  KEY `admin_id` (`admin_id`)
        ...
        $applyTemplate = IdcsmartWithdrawLogic::getDefaultConfig('idcsmart_withdraw_apply_notice_template');
        if(!empty($applyTemplate)){
            foreach ($applyTemplate as $key=>$template){
                $template['name'] = $key;
                notice_action_create($template);
            }
        }
	...
	public function uninstall()
	{
        ...
        $templatesTwo = IdcsmartWithdrawLogic::getDefaultConfig('idcsmart_withdraw_apply_notice_template');
        foreach ($templatesTwo as $key=>$template){
            notice_action_delete($key);
        }
	...
	public function clientWithdraw($param)
	{
		...
		if (!$IdcsmartWithdrawValidate->scene('withdraw')->check($param)){
			return ['status' => 400 , 'msg' => lang_plugins($IdcsmartWithdrawValidate->getError())];
		}
