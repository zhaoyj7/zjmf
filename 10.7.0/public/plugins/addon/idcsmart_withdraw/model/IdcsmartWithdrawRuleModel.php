...
class IdcsmartWithdrawRuleModel extends Model
{
    ...
    public function idcsmartWithdrawRuleCredit($app = '')
    {
        ...
        if($app=='home'){
            $methods = IdcsmartWithdrawMethodModel::field('id,name')->whereIn('id', $idcsmartWithdrawRule['method'])->select()->toArray();
            $param = request()->param();
            if (!empty($param['scene']) && $param['scene']=='recommend'){
                array_unshift($methods, ['id'=>0,'name'=>lang_plugins('idcsmart_withdraw_rule_method_credit')]);
            }
            $idcsmartWithdrawRule['method'] = $methods;
        }
