...
class IdcsmartWithdrawModel extends Model
{
   ...
   /**
    ...
    public function idcsmartWithdrawList($param, $app = 'admin')
    {
        ...
        if($app=='admin'){
            ...
            $totalFee = $this->alias('aiw')
                ...
                ->where($where)
                ->sum('aiw.fee');
            $amountAll = $totalAmount;
            $totalAmount = bcsub($totalAmount, $totalFee, 2);
        }

        $list = $this->alias('aiw')
            ->field('aiw.id,aiw.amount,aiw.fee,aiwm.name method,aiw.card_number,aiw.name,aiw.account,aiw.status,aiw.reason,aiw.create_time,aiw.client_id,c.username,c.company,aiw.source,t.transaction_number,aiw.addon_idcsmart_withdraw_method_id')
            ->leftJoin('client c', 'c.id=aiw.client_id')
        ...
        if($app=='admin'){
            return ['list' => $list, 'count' => $count, 'total_amount' => $totalAmount, 'page_total_amount' => $pageTotalAmount,'amount'=>amount_format($amount),'amount_all'=>amount_format($amountAll)];
        }else{
    ...
    public function idcsmartWithdrawAudit($param)
    {
        ...
        if($param['status']==1){
            // wyh 20250926 修改，提现至余额，直接已汇款
            if ($idcsmartWithdraw['addon_idcsmart_withdraw_method_id']==0){
                $this->update([
                    'status' => 3,
                    'update_time' => time()
                ], ['id' => $param['id']]);
                update_credit([
                    'type' => 'Recommend',
                    'amount' => bcsub($idcsmartWithdraw['amount'], $idcsmartWithdraw['fee'],2),
                    'notes' => lang_plugins('recommend_withdraw'),
                    'client_id' => $idcsmartWithdraw['client_id'],
                ]);
            }
            $params = [
    ...
    public function idcsmartWithdraw($param)
    {
        ...
        $withdrawMethod = IdcsmartWithdrawMethodModel::find($param['method_id']);
        $withdrawAccount = '';
        if(!empty($param['card_number'])){
            // 银行卡号脱敏处理
            $cardNumber = $param['card_number'];
            if(strlen($cardNumber) > 8){
                $withdrawAccount = substr($cardNumber, 0, 4) . '****' . substr($cardNumber, -4);
            }else{
                $withdrawAccount = $cardNumber;
            }
            ...
            if(strpos($account, '@') !== false){
                ...
                if(strlen($parts[0]) > 3){
                    $withdrawAccount = substr($parts[0], 0, 3) . '***@' . $parts[1];
                }else{
                    $withdrawAccount = $account;
                }
            }else{
                if(strlen($account) > 7){
                    $withdrawAccount = substr($account, 0, 3) . '****' . substr($account, -4);
                }else{
                    $withdrawAccount = $account;
                }
            }
        ...
        system_notice([
            ...
            'email_description' => lang_plugins('cash_withdrawal_apply_notice_send_mail'),
            'sms_description' => lang_plugins('cash_withdrawal_apply_notice_send_sms'),
            'task_data' => [
                'client_id' => $clientId,
                'template_param' => [
                    'withdraw_id' => $idcsmartWithdraw->id,
                    'client_name' => $client['username'],
                    'withdraw_amount' => $currencyPrefix . $param['amount'] . $currencySuffix,
                    'withdraw_method' => $withdrawMethod['name'] ?? '',
                    'withdraw_account' => $withdrawAccount,
                    'apply_time' => date('Y-m-d H:i:s'),
                    'admin_url' => request()->domain() . '/admin/plugin/idcsmart_withdraw/index',
                ],
            ],
        ]);
