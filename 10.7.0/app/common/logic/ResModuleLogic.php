        ...
        if ($res['status']==200){
        	...
        	if($res['data']['supplier']['type']=='whmcs'){
                ...
                if ($this->upstreamProduct['mode']=='sync'){
                    ...
                    $upstream_configoption = (new ModuleLogic())->exchangeParams($product,$upstream_configoption,'create_account',$HostModel);
                }
        		$param = [
...
//                                'price_basis' => $this->upstreamProduct['price_basis']??'agent',
                    ...
                    if ($res['status']==200){
                        if (isset($res['data']['order_id']) && $res['data']['order_id']){ # 已在上游下单,但未支付
                            ...
                            if (!empty($res['data']['customfields']['agent_use_coin'])){
                                $resultCoin = idcsmart_api_curl($apiId,'/console/v1/coin/pay',[
                                    'auto'=>1,
                                    'coin_coupon_ids'=>[],
                                ...
                                if ($resultCoin['status']==200){
                                    $res['data']['amount'] = $resultCoin['data']['amount']??0;
                                }
                            }

                            // 亏本交易拦截检查
                            $checkResult = checkLossTrade($HostModel['order_id'], $res['data']['amount']??0, 'new_order', '', $id);
                            if (!$checkResult['pass']) {
                                throw new \Exception($checkResult['msg']);
                            }
                            ...
                            # 支付(余额支付失败后回退信用额支付)
                            $res = $this->payWithCreditLimitFallback($apiId,$payData);
                        }else{
                            ...
                            // 可以修改参数调用上游参数
                            hook('before_res_module_create_account', [
                                'upstream_product'  => $this->upstreamProduct,
                                'host'              => $HostModel,
                            ...
                            if ($res['status']==200){
                                ...
                                if ($res['status']==200){
                                    ...
                                    if ($res['data']['amount']>0){ // 处理需要支付的
                                        // 使用平台币支付
                                        if (!empty($res['data']['customfields']['agent_use_coin'])){
                                            $result = idcsmart_api_curl($apiId,'/console/v1/coin/pay',[
                                                'auto'=>1,
                                                'coin_coupon_ids'=>[],
                                                'order_id' => $res['data']['order_id']??'',
                                                'use' => 1,
                                            ...
                                            if ($result['status']==200){
                                                $res['data']['amount'] = $result['data']['amount']??0;
                                            }
                                        }
                                        
                                        // 亏本交易拦截检查
                                        $checkResult = checkLossTrade(
                                            $HostModel['order_id'],
                                            $res['data']['amount'],
                                            'new_order',
                                            '',
            ...
            if ($res['status']==200){
	         ...
	         if ($res['status']==200){
	             if ($res['code']=='Unpaid'){ # 未支付
	                 ...
	                 # 使用余额
	                 $res1 = idcsmart_api_curl($apiId,'/console/v1/credit',$creditData,30,'POST');
                     if ($res1['status']==200 || ($res1['status']==400 && !empty($res1['data']['no_credit']))){
                         // 使用平台币支付
                         if (!empty($res1['data']['customfields']['agent_use_coin'])){
                             $result = idcsmart_api_curl($apiId,'/console/v1/coin/pay',[
                                 'auto'=>1,
                                 'coin_coupon_ids'=>[],
        ...
        if ($res['status']!=200){
            ...
            system_notice([
                ...
                'task_data' => [
                    ...
                    'host_id'   => $id,
                    'template_param'=>[
                        'action' => lang('module_button_renew'),
                    ],
                ],
            ]);
        }
	...
	public function cartCalculatePrice($ProductModel, $params = [], $qty=1, $scene="cal_price",$create=false,$cartSettle=false)
	{
        ...
        elseif ($supplier['type']=='finance'){
            ...
            if ($res['status']==200){
                ...
                $res['data']['description'] = $description;
                $res['data']['base_price'] = $res['data']['price'];
                // $res['data']['content'] = $description;
                // 删除不必要返回
	        ...
	        if ($res['status']==200){
                ...
                $res['data']['renew_price'] = ($res['data']['renew_price'] ?? 0) * $supplier['rate'];
                $res['data']['base_price'] = ($res['data']['base_price'] ?? 0) * $supplier['rate'];
                if(isset($res['data']['discount_order_price'])){
                    $res['data']['discount_order_price'] = ($res['data']['discount_order_price'] ?? $res['data']['base_price'] ?? 0) * $supplier['rate']; // 订购可应用用户等级部分
                }
                if(isset($res['data']['discount_renew_price'])){
                    $res['data']['discount_renew_price'] = ($res['data']['discount_renew_price'] ?? $res['data']['renew_price'] ?? 0) * $supplier['rate']; // 续费可应用用户等级部分
                }
                // 根据利润计算本地价格(到本地的基础价格，需要考虑利润为负的情况！)
                if ($this->upstreamProduct['profit_type']==1){
                    // 利润数据，返回给购物车使用
                    ...
                    // 固定利润：删除折扣价格字段，让系统重新计算
                    if(isset($res['data']['discount_order_price'])) unset($res['data']['discount_order_price']);
                }else{
                    // 按比例利润：折扣价格直接乘以倍数
                    $profitAndBasePercent = $res['data']['profit'] = bcadd(1,$this->upstreamProduct['profit_percent']/100,4);
                    $res['data']['price'] = bcmul($res['data']['price'],$profitAndBasePercent,2)>0?bcmul($res['data']['price'],$profitAndBasePercent,2):0;
                    ...
                    // 订购折扣价格按比例计算
                    if(isset($res['data']['discount_order_price'])){
                        $res['data']['discount_order_price'] = bcmul($res['data']['discount_order_price'],$profitAndBasePercent,2)>0?bcmul($res['data']['discount_order_price'],$profitAndBasePercent,2):0;
                    }
                }
                // 续费利润(需要考虑利润为负的情况！)
                if ($this->upstreamProduct['renew_profit_type']==1){
                    ...
                    // 固定利润：删除续费折扣价格字段
                    if(isset($res['data']['discount_renew_price'])) unset($res['data']['discount_renew_price']);
                }else{
                    // 按比例利润：续费折扣价格按比例计算
                    $renewProfitAndBasePercent = bcadd(1,$this->upstreamProduct['renew_profit_percent']/100,4);
                    ...
                    // 续费折扣价格按比例计算
                    if(isset($res['data']['discount_renew_price'])){
                        $res['data']['discount_renew_price'] = bcmul($res['data']['discount_renew_price'],$renewProfitAndBasePercent,2)>0?bcmul($res['data']['discount_renew_price'],$renewProfitAndBasePercent,2):0;
                    }
                }
                $fixedProfit = $this->upstreamProduct['profit_percent'];
                $description = '';
        ...
        if ($res['status']==200 && isset($param['is_downstream']) && $param['is_downstream']==1 && $priceAgent){
            ...
            if (!empty($plugin)){
                ...
                if(isset($res['data']['discount_order_price'])){
                    $discountOrderPriceDiscount = $IdcsmartClientLevelModel->productDiscount([
                        'id' => $ProductModel['id'],
                        'amount' => $res['data']['discount_order_price']
                    ]);
                    $res['data']['discount_order_price'] = bcsub($res['data']['discount_order_price'], $discountOrderPriceDiscount, 2)>0?bcsub($res['data']['discount_order_price'], $discountOrderPriceDiscount, 2):0;
                }
                if(isset($res['data']['discount_renew_price'])){
                    $discountRenewPriceDiscount = $IdcsmartClientLevelModel->productDiscount([
                        'id' => $ProductModel['id'],
                        'amount' => $res['data']['discount_renew_price']
                    ]);
                    $res['data']['discount_renew_price'] = bcsub($res['data']['discount_renew_price'], $discountRenewPriceDiscount, 2)>0?bcsub($res['data']['discount_renew_price'], $discountRenewPriceDiscount, 2):0;
                }
	...
	public function downloadResource($ProductModel){
        ...
        $supplier = SupplierModel::find($apiId);
        if ($supplier['type']=='whmcs'){
            $res = idcsmart_api_curl($apiId, 'product_detail', ['productid' => $this->upstreamProduct['upstream_product_id']] ,30, 'POST');
...
//                });
    ...
    public function syncAccount(HostModel $HostModel)
    {
        ...
        if($supplier['type']=='whmcs'){
            // 暂不支持
             $res = idcsmart_api_curl($apiId,"host_detail",['hosting_id' => $upstreamHost['upstream_host_id']],30,'POST');
             if ($res['status']==200){
                 $result['status'] = 200;
                 ...
                 $result['data'] = [
                     ...
                     'status' => $res['data']['status']??$HostModel['status'],
                     'name' => $res['data']['hostname']??$HostModel['name'],
	...
	/**
	 * 时间 2025-03-12
	 * @title 上游支付(余额支付失败后回退信用额支付)
	 * @desc  先尝试余额支付,失败后自动尝试信用额支付
	 * @author wyh
	 * @version v1
	 * @param   int apiId - 供应商ID require
	 * @param   array payData - 支付参数(id,gateway) require
	 * @return  array
	 */
	private function payWithCreditLimitFallback($apiId, $payData)
	{
		// 余额支付
		$result = idcsmart_api_curl($apiId, '/console/v1/pay', $payData, 30, 'POST');
		// 余额支付失败，尝试信用额支付
		if ($result['status'] != 200) {
			// 先取消余额占用
			idcsmart_api_curl($apiId, '/console/v1/credit', [
				'id' => $payData['id'] ?? 0,
				'use' => 0,
			], 30, 'POST');
			// 再尝试信用额支付
			...
			$creditLimitResult = idcsmart_api_curl($apiId, '/console/v1/credit_limit/pay', $creditLimitPayData, 30, 'POST');
