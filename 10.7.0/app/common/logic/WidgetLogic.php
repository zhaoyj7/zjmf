...
class WidgetLogic
{
    ...
    {
        $this->loadWidget();
        cache($this->cacheKey, serialize($this->widgets), 3600);
        return $this->widgets;
    }
     ...
     * @version v1
     */
    public function loadCacheWidget()
    {
        $widgets = cache($this->cacheKey);
        if(empty($widgets)){
            $widgets = $this->cacheWidgets();
        }else{
