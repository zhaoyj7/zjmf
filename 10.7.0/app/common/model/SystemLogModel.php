...
class SystemLogModel extends Model
{
    ...
    public function systemLogList($param)
    {
        ...
        $where = function (Query $query) use($param, $app) {
            ...
            }else if($param['type']=='cron'){
                $query->where('user_type','cron');
            }else if($param['type']=='task'){
                $query->where('user_type','task');
            }else{
...
//    	$count = $this->where($where)->count();
        $count = 100000000;
    	$logs = $this->field('id,description,create_time,ip,user_type,user_id,user_name,port')
    		->where($where)
