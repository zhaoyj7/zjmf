...
class SelfDefinedFieldModel extends Model
{
    ...
    public function selfDefinedFieldCreate($param)
    {
        if($param['type'] == 'product'){
            ...
            $param['relid'] = 0;
            $param['is_global'] = $param['is_global'] ?? 0;
        }
        ...
        $selfDefinedField = $this->create($param, ['type','relid','field_name','is_required','field_type','description','regexpr','field_option','show_order_page','show_order_detail','show_client_host_detail','show_admin_host_detail','show_client_host_list','show_admin_host_list','create_time','explain_content','is_global']);
    ...
    public function isGlobalUpdate($param)
    {
        $selfDefinedField = $this->find($param['id']);
        if(empty($selfDefinedField)){
            return ['status'=>400, 'msg'=>lang('self_defined_field_not_found')];
        ...
        if($selfDefinedField['is_global'] == $param['is_global']){
        ...
        try{
            ...
            $this->update($param, ['id'=>$selfDefinedField['id'] ], ['update_time','is_global']);

            if($param['is_global'] == 1){
                if(isset($param['remove']) && $param['remove']==1){
                    $productId = $this->alias('a')
                            ->leftJoin('self_defined_field_link b','b.self_defined_field_id=a.id')
                            ->leftJoin('product c','c.product_group_id=b.product_group_id')
                            ->where('a.id', $param['id'])
                            ->column('c.id');
                    if(!empty($productId)){
                        $fieldId = $this
                            ->where('field_name', $selfDefinedField['field_name'])
                            ->where('type', 'product')
                            ->whereIn('relid', $productId)
                            ->column('id');
                        if(!empty($fieldId)){
                            $this->whereIn('id', $fieldId)->delete();
                            $SelfDefinedFieldValueModel = new SelfDefinedFieldValueModel();
                            $SelfDefinedFieldValueModel->whereIn('self_defined_field_id', $fieldId)->update(['self_defined_field_id' => $param['id']]);
                        }
                    }
                    $description = lang('log_product_group_self_defined_field_is_global_enable_success_with_delete', [
                        '{name}'  => $selfDefinedField['field_name'],
                    ]);
                }else{
                    $description = lang('log_product_group_self_defined_field_is_global_enable_success', [
                        '{name}'  => $selfDefinedField['field_name'],
                    ]);
                }
            }else{
                $description = lang('log_product_group_self_defined_field_is_global_disable_success', [
                    '{name}'  => $selfDefinedField['field_name'],
                ]);
            }
            active_log($description);
    ...
    public function selfDefinedFieldList($param)
    {
        ...
        $list = $this
                ->field('id,field_name,field_type,description,regexpr,field_option,is_required,show_order_page,show_order_detail,show_client_host_detail,show_admin_host_detail,show_client_host_list,show_admin_host_list,upstream_id,explain_content,is_global')
                ->where($where)
                ->order('order,id', 'asc')
                ->select()
                ->toArray();
        ...
        if($param['type']=='product'){
            $SelfDefinedFieldLinkModel = new SelfDefinedFieldLinkModel();
            $globalList = $SelfDefinedFieldLinkModel->alias('b')
                ->field('a.id,a.field_name,a.field_type,a.description,a.regexpr,a.field_option,a.is_required,a.show_order_page,a.show_order_detail,a.show_client_host_detail,a.show_admin_host_detail,a.show_client_host_list,a.show_admin_host_list,a.upstream_id,a.explain_content')
                ->leftJoin('self_defined_field a', 'b.self_defined_field_id=a.id')
                ->leftJoin('product c', 'b.product_group_id=c.product_group_id')
                ->where('c.id', $param['relid'])
                ->where('a.type', 'product_group')
                ->where('a.is_global', 1)
                ->order('a.order,a.id', 'asc')
                ->select()
                ->toArray();

            return ['list'=>$list, 'count'=>$count, 'global_list'=>$globalList];
        }else{
    ...
    public function showOrderPageField($param)
    {
        ...
        $SelfDefinedFieldLinkModel = new SelfDefinedFieldLinkModel();
        $globalFieldIds = $SelfDefinedFieldLinkModel->alias('a')
                ->field('a.self_defined_field_id')
                ->leftJoin('product b', 'b.product_group_id=a.product_group_id')
                ->leftJoin('self_defined_field c', 'c.id=a.self_defined_field_id')
                ->where('b.id', $param['id'])
                ->where('c.type', 'product_group')
                ->where('c.is_global', 1)
                ->select()
                ->toArray();
        $globalFieldIds = array_column($globalFieldIds, 'self_defined_field_id');

        $data2 = $this
                ->field($field)
                ->whereIn('id', $globalFieldIds)
                ->where('show_order_page', 1)
                ->order('order,id', 'asc')
                ->select()
                ->toArray();
        foreach($data2 as $k=>$v){
            ...
            $v['field_name'] = $multiLanguage['field_name'] ?? $v['field_name'];
            $v['description'] = $multiLanguage['description'] ?? $v['description'];
            $v['explain_content'] = $multiLanguage['explain_content'] ?? $v['explain_content'];
            $data[] = $v;
        }        
    ...
    public function showAdminHostDetailField($param)
    {
        ...
        $data2 = $this
                ...
                ->field('sdf.id,sdf.field_name,sdf.field_type,sdf.description,sdf.field_option,sdf.is_required,sdfv.value')
                ->leftJoin('self_defined_field_value sdfv', 'sdf.id=sdfv.self_defined_field_id AND sdfv.relid='.$id)
                ->leftJoin('self_defined_field_link sdfl', 'sdf.id=sdfl.self_defined_field_id')
                ->leftJoin('product p', 'p.product_group_id=.sdfl.product_group_id')
                ->where('p.id', $productId)
                ->where('sdf.type', 'product_group')
                ->where('sdf.is_global', 1)
                ->where('sdf.show_admin_host_detail', 1)
                ->withAttr('value', function($value, $row){
                    if($row['field_type'] == 'tickbox'){
                        $value = $value == '1' ? '1' : '0';
    ...
    public function getHostListSelfDefinedFieldValue($param)
    {
        ...
        $where = [];
        $where[] = ['c.id', 'IN', $param['product_id']];
        $where[] = ['a.type', '=', 'product_group'];
        $where[] = ['a.show_client_host_list', '=', 1];
        $where[] = ['a.is_global', '=', 1];

        $selfDefinedField2 = $this->alias('a')
                            ->field('a.id,a.field_name,a.field_type')
                            ->leftJoin('self_defined_field_link b', 'b.self_defined_field_id=a.id')
                            ->leftJoin('product c', 'c.product_group_id=b.product_group_id')
                            ->where($where)
                            ->withAttr('field_name', function($val){
                            ...
                            ->select()
                            ->toArray();
        $selfDefinedField = array_merge($selfDefinedField, $selfDefinedField2);
