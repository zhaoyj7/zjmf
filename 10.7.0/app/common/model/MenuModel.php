...
class MenuModel extends Model
{
    ...
    public function getHomeMenu()
    {
        ...
        foreach ($pluginNavs as $key => $value) {
            foreach ($plugins as $k => $v) {
                $plugins[$k]['navs'] = $plugins[$k]['navs'] ?? [];
                if($value['plugin']==$v['name'] && $value['module']==$v['module'] && !empty($value['url'])){
                    $plugins[$k]['navs'][] = ['id' => $value['id'], 'name' => lang_plugins($value['name']), 'url' => $value['url']];
                }
            }
        }
        ...
        $menus = $this->field('id,menu_type type,name,language,url,second_reminder,icon,nav_id,parent_id,module,product_id,res_module,show_quick_order,quick_order_url,is_cross_module,select_field')
            ->where('type', 'home')
            ->order('order','asc')
        ...
        if(empty($menus)){
            ...
            foreach ($menus as $key => $value) {
                ...
                $menus[$key]['is_cross_module'] = 0;
                $menus[$key]['select_field'] = [];
            }
        }
        // 将数组转换成树形结构
        $tree = [];
        if (is_array($menus)) {
            ...
            foreach ($menus as $key => $data) {
                ...
                $menus[$key]['module'] = !empty($data['module']) ? explode(',', $data['module']) : [];
                $menus[$key]['select_field'] = !empty($data['select_field']) ? explode(',', $data['select_field']) : [];
                $refer[$data['id']] = &$menus[$key];
            }
            foreach ($menus as $key => $data) {
                // 判断是否存在parent  获取他的父类id
        ...
        foreach ($param['menu'] as $key => $value) {
            if(in_array($value['type'], ['system', 'plugin'])){
                ...
                if(!empty($value['module'])){
                    if(!empty(array_diff($value['module'], $module))){
                    ...
                    $products = ProductModel::alias('p')
                        ...
                        //->where('p.hidden', 0)
                        ->whereIn('s.module|ss.module', $value['module'])
            ...
            if(isset($value['child'])){
                foreach ($value['child'] as $k => $v) {
                    if(in_array($v['type'], ['system', 'plugin'])){
                        ...
                        // 验证模块
                        if(!empty($v['module'])){
                            if(!empty(array_diff($v['module'], $module))){
                                return ['status' => 400, 'msg' => lang('module_error')];
                            }
                            $products = ProductModel::alias('p')
                            ->leftjoin('server s','p.type=\'server\' AND p.rel_id=s.id')
        ...
        try {
            ...
            foreach ($param['menu'] as $key => $value) {
                $menu = $this->create([
                    ...
                    'is_cross_module'   => $value['is_cross_module']??0,
                    'select_field'      => implode(',', $value['select_field']??[]),
                ]);
                $order++;
                if(isset($value['child'])){
    ...
    public function saveChild($type = '', $parentId = 0, $child = [], $order = 0)
    {
        foreach ($child as $k => $v) {
            $menu = $this->create([
                ...
                'is_cross_module'   => $v['is_cross_module']??0,
                'select_field'      => implode(',', $v['select_field']??[]),
            ]);
            $order++;
            if(isset($v['child'])){
    ...
    public function homeMenu(){
        ...
        if(!empty($clientId)){
            ...
            $menus = $this->alias('m')
                ->field('m.id,m.name,m.language,m.url,m.second_reminder,m.icon,m.parent_id,n.url nav_url,m.menu_type,n.plugin,m.show_quick_order,m.quick_order_url,m.is_cross_module,m.select_field')
                ->leftjoin('nav n', 'n.id=m.nav_id')
                ->where('m.type', 'home')
                ->order('m.order','asc')
                ->select()
            ...
            foreach ($menus as $key => $data) {
                ...
                $menus[$key]['name'] = $data['language'][$language] ?? $data['name'];
                $menus[$key]['url'] = $data['nav_url'] ?? $data['url'];
                $menus[$key]['select_field'] = !empty($value['select_field']) ? explode(',', $value['select_field']) : [];
                if($data['menu_type']=='module' || $data['menu_type']=='res_module'){
                    if($data['is_cross_module']==1){
                        $menus[$key]['url'] = 'crossModule.htm?m='.$data['id'];
                    }else{
