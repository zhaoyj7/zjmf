...
class UpstreamOrderModel extends Model
{
    ...
    public function orderList($param)
    {
        ...
        $orderItems = OrderItemModel::alias('oi')
            ->field('oi.order_id,oi.type,h.id,h.name,h.billing_cycle,h.billing_cycle_name,p.name product_name,oi.description,oi.amount')
        ...
        foreach ($orders as $key => $order) {
            ...
            $orders[$key]['cost'] = amount_format(bcsub($order['amount'],$orders[$key]['profit'],2)); // 处理金额格式
