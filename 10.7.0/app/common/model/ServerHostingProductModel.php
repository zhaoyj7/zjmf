...
class ServerHostingProductModel extends Model
{
    ...
    public function productOrder($param)
    {
        ...
        // 验证ID数组
        $count = $this->whereIn('id', $ids)->count();
        if($count != count($ids)){
        ...
        try {
            foreach($ids as $key=>$id){
                $this->update(['order'=>$key+1], ['id'=>$id]);
            }

            # 记录日志
            active_log(lang('log_order_server_hosting_product', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#']));
