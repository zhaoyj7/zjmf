...
class TrademarkRegisterProductModel extends Model
{
     ...
     * @author theworld
     * @version v1
     * @param array param.id - 商品ID required
     * @return int status - 状态码,200成功,400失败
     * @return string msg - 提示信息
     */
    public function productOrder($param)
    {
        $id = $param['id'] ?? [];
        if(empty($id)){
        ...
        $product = $this->column('id');
        if(count($id)!=count($product) || count($id)!=count(array_intersect($product, $id))){
            return ['status'=>400, 'msg'=>lang('trademark_register_product_not_exist')];
        }
        ...
        try {
            ...
            # 记录日志
            active_log(lang('log_order_trademark_register_product', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#']));
