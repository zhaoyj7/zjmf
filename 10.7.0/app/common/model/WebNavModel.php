...
class WebNavModel extends Model
{
     ...
     * @param string param.language - 语言zh-cn,en-us
    ...
    public function navList($param)
    {
        ...
        $list = $this->field('id,parent_id,name,description,file_address,icon,show,blank')
            ->where('theme', $theme)
        ...
        foreach ($list as $key => $value) {
            if(empty($value['parent_id'])){
                $first[$value['id']] = ['id' => $value['id'], 'name' => $value['name'], 'file_address' => $value['file_address'], 'show' => $value['show'], 'blank' => $value['blank'], 'children' => []];
            }
        }
        foreach ($list as $key => $value) {
     ...
     * @param int param.blank - 是否打开新窗口 required
    ...
    public function updateNav($param)
    {
        ...
        try {
            ...
            $param['blank'] = lang('whether_'.$param['blank']);
            $nav['blank'] = lang('whether_'.$nav['blank']);
     ...
     * @version v1
     * @param int param.id - 导航ID required
     * @param int param.blank - 是否打开新窗口0否1是 required
     * @return int status - 状态码,200成功,400失败
     * @return string msg - 提示信息
     */
    public function navBlank($param)
    {
        // 验证导航ID
        $nav = $this->find($param['id']);
        if(empty($nav)){
    ...
    public function firstNavOrder($param)
    {
        ...
        $nav = $this->where('parent_id', 0)->where('theme', $theme)->where('language', $param['language'])->column('id');
     ...
     * @return string list[].name - 名称
     * @return string list[].file_address - 文件地址
     * @return int list[].blank - 是否打开新窗口0否1是
     * @return array list[].children - 二级导航
     * @return int list[].children[].id - 二级导航ID
     * @return int list[].children[].parent_id - 父导航ID
     * @return string list[].children[].name - 名称
     ...
     * @return string list[].children[].file_address - 文件地址
     * @return string list[].children[].icon - 图标
     * @return int list[].children[].blank - 是否打开新窗口0否1是
     */
    public function webHeaderNav($language='zh-cn')
    {
        $theme = configuration('web_theme');

        $list = $this->field('id,parent_id,name,description,file_address,icon,blank')
            ->where('show', 1)
            ->where('theme', $theme)
        ...
        $first = [];
        foreach ($list as $key => $value) {
            if(!empty($value['file_address'])){
                if(stripos($value['file_address'], 'https://')===false && stripos($value['file_address'], 'http://')===false){
            ...
            if(empty($value['parent_id'])){
                $first[$value['id']] = ['id' => $value['id'], 'name' => $value['name'], 'file_address' => $value['file_address'], 'blank' => $value['blank'], 'children' => []];
            }
        }
        foreach ($list as $key => $value) {
            if(!empty($value['parent_id']) && isset($first[$value['parent_id']])){
