...
//			}
			...
			foreach($params as $name=>$param){
				...
				if(!empty($param['sms_name'])){
					...
					if(!empty($param['sms_template']) && (!isset($sms_cn_template[$param['sms_template']]) || $sms_cn_template[$param['sms_template']]!=0)){
				...
				if(!empty($param['sms_global_name'])){
					...
					if(!empty($param['sms_global_template']) && (empty($sms_global_template[$param['sms_global_template']]) || $sms_global_template[$param['sms_global_template']]!=1)){
				...
				if(!empty($param['email_name'])){
					...
					if(!empty($param['email_template']) && !in_array($param['email_template'],$email_template)){
    ...
    public function batchUpdate($param)
    {
        if (empty($param['name']) || !is_array($param['name'])){
            return ['status' => 400, 'msg' => lang('param_error')];
        }
        if (empty($param['sms_name']) && empty($param['email_name']) && empty($param['sms_global_name'])){
            return ['status' => 400, 'msg' => lang('param_error')];
        }
        $data = [];
        if (!empty($param['sms_name'])){
        ...
        if (!empty($param['email_name'])){
            $data[] = $param['email_name'];
        }
        if (!empty($param['sms_global_name'])){
            $data[] = $param['sms_global_name'];
        }
        $PluginModel = new PluginModel();
        $count = $PluginModel->whereIn('name',array_unique($data))->where('status',1)->count();
        if ($count != count(array_unique($data))){
            return ['status' => 400, 'msg' => lang('param_error')];
        }
        $count1 = $this->whereIn('name',$param['name'])->count();
        if ($count1 != count($param['name'])){
            return ['status' => 400, 'msg' => lang('param_error')];
        }
        $this->startTrans();
        try {
            ...
            // 国内
            if (!empty($param['sms_name'])){
                $smsTemplates = $SmsTemplateModel->where('sms_name',$param['sms_name'])
                    ->where('type',0)
                    ->where('notice_setting_name','<>','')
                    ->select()
                    ->toArray();
                $smsTemplatesMap = array_column($smsTemplates,'id','notice_setting_name');
            }
            // 国外
            if (!empty($param['sms_global_name'])){
                $smsGlobalTemplates = $SmsTemplateModel->where('sms_name',$param['sms_global_name'])
                    ->where('type',1)
                    ->where('notice_setting_name','<>','')
                    ->select()
                    ->toArray();
                $smsGlobalTemplatesMap = array_column($smsGlobalTemplates,'id','notice_setting_name');
            }
            // 邮件
            $EmailTemplateModel = new EmailTemplateModel();
            if (!empty($param['email_name'])){
                $emailTemplates = $EmailTemplateModel->where('notice_setting_name','<>','')->select()->toArray();
                $emailTemplatesMap = array_column($emailTemplates,'id','notice_setting_name');
            }
            foreach ($param['name'] as $name){
                if (!empty($param['sms_name'])){
                    $this->where('name',$name)->update([
                        'sms_name' => $param['sms_name'],
                        'sms_template' => $smsTemplatesMap[$name]??0,
                    ]);
                }
                if (!empty($param['email_name'])){
                    $this->where('name',$name)->update([
                        'email_name' => $param['email_name'],
                        'email_template' => $emailTemplatesMap[$name]??0,
                    ]);
                }
                if (!empty($param['sms_global_name'])){
                    $this->where('name',$name)->update([
                        'sms_global_name' => $param['sms_global_name'],
                        'sms_global_template' => $smsGlobalTemplatesMap[$name]??0,
                    ]);
                }
            }
            $this->commit();
