...
class NavModel extends Model
{
     ...
     * @author hh
     * @version v1
     * @param array nav - 导航数据 require
     * @param string nav[].name - 名称 require
     * @param string nav[].url - 网址 require
     * @param string nav[].in - 定义插件导航位置:可定义导航在某个一级导航之下,默认会放置在此一级导航最后的位置
     * @param string nav[].icon - 导航图标
     * @param int nav[].parent_id - 父ID
     * @param string nav[].menu_name - 导航名称
     * @param string type - 类型:admin后台,home前台
     * @return string msg - 提示信息
     */
    public function createSystemNav(array $nav, string $type = 'admin')
    {
        if (isset($nav['in']) && !empty($nav['in']) && isset($nav['url']) && !empty($nav['url']) && (!isset($nav['child']) || empty($nav['child']))){
            $firstNav = $this->where('name',$nav['in'])->where('parent_id',0)->find();
            if (!empty($firstNav)){
        ...
        $viewSuffix = '.htm';
        if(strpos($nav['url'], $viewSuffix) === false){
            $url = $nav['url'].$viewSuffix;
        }else{
            $url = $nav['url'];
        }
        ...
        foreach ($child as $item){
            $item['parent_id'] = $object->id;
            $this->createSystemNav($item, $type);
        }
