...
class OauthModel extends Model
{
    ...
    {   
        ...
        $_SESSION['oauth']['login_method'] = $plugin['title'];
        ...
        if(empty($oauth)){
            ...
            if(!empty($clientId)){
                ...
                if(!empty($client)){
                    ...
                    $_SESSION['oauth']['client_name'] = $client->username;
                    // 跳转至绑定页面
                    $result = [
                        'status'    => 200,
                        'msg'       => lang('success_message'),
    ...
    public function checkToken($param)
    {
        ...
        if(isset($oauth['jwt'])){
            ...
            // 正常登录日志
            $log = lang('log_client_login',['{client}'=>'client#'.$oauth['client_id'].'#'.$oauth['client_name'].'#','{type}'=>$oauth['login_method']]);

            $request = request();
            $request->client_id = $oauth['client_id'];
            $request->client_name = $oauth['client_name'];
            active_log($log,'login',$oauth['client_id']);
    ...
    public function bind($param)
    {
        ...
        $log = lang('log_client_login',['{client}'=>'client#'.$clientId.'#'.$client['username'].'#','{type}'=>$oauth['login_method'] ?? lang('nav_oauth') ]);

        $request = request();
        $request->client_id = $clientId;
        $request->client_name = $client['username'];
        active_log($log,'login',$clientId);
