...
class AppMarketController extends AdminBaseController
{
     ...
     * @method POST
     * @return string market_url - desc:商店自动登录url
     */
	public function setToken(){
		$adminId = get_admin_id();
		$adminRoleLink = AdminRoleLinkModel::where('admin_id', $adminId)->find();
		if(empty($adminRoleLink) || $adminRoleLink['admin_role_id']!=1){
			$token = '';
		}else{
     ...
     * @method GET
     * @param string token - desc:验证token validate:required
     * @param string license - desc:授权码 validate:optional
     */
     ...
     * @method GET
     * @return array list - desc:应用列表
     * @return int list[].id - desc:应用ID
     * @return string list[].uuid - desc:应用标识
     * @return string list[].old_version - desc:应用当前版本
     * @return string list[].version - desc:应用最新版本
     * @return string list[].support_version - desc:支持的系统版本
     * @return string list[].type - desc:应用分类 addon插件 captcha验证码接口 certification实名接口 gateway支付接口 mail邮件接口 sms短信接口 server模块 template主题 oauth第三方登录 sub_server子模块 widget首页挂件 oss对象存储
     * @return string list[].error_msg - desc:错误信息 不为空代表不可下载和升级插件
     * @return int upgrade - desc:是否存在可升级应用 0否 1是
     */
     ...
     * @return string old_version - desc:主题当前版本
     * @return string version - desc:主题最新版本
     * @return string support_version - desc:支持的系统版本
     * @return string description - desc:版本描述
     * @return int upgrade - desc:是否可升级 0否 1是
     * @return string error_msg - desc:错误信息 不为空代表不可下载和升级插件
     */
     ...
     * @method GET
     * @param int id - desc:应用ID validate:required
     * @return string type - desc:应用分类 addon插件 captcha验证码接口 certification实名接口 gateway支付接口 mail邮件接口 sms短信接口 server模块 template主题 oauth第三方登录 sub_server子模块 widget首页挂件 oss对象存储
     */
     ...
     * @author theworld
     * @version v1
     * @param string filepath - desc:文件路径
     * @param string path - desc:解压目标路径
     ...
     * @author theworld
     * @version v1
     * @param string url - desc:下载链接地址
     * @param string file_name - desc:目标路径
     * @return mixed
     */
