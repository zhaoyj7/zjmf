...
class PluginModel extends Model
{
    ...
    public function pluginList($param)
    {
        ...
        $moduleInDb = $this->where('module',$module)
            ->field('id,status,name,title,author,author_url,version,description,help_url,description_url,config')
            ->order('order','asc')
            ->select()
            ->toArray();
        ...
        foreach ($moduleInDb as $plugin){
            ...
            if ($module=='gateway'){
                $plugin['pay_name'] = json_decode($plugin['config'],true)['module_name']??'';
            }
        ...
        foreach ($dirs as $k=>$dir) {
            ...
            if (!isset($plugins[$pluginDir])) { # 数据库未设置此插件
                ...
                $plugins[$pluginDir]['pay_name'] = $obj->info['title']??'';
            }
        }
    ...
    public function install($param)
    {
			...
			if($module=="sms"){
				if(is_array($installSuccess) && count($installSuccess)>0){
					...
					foreach($installSuccess2 as $v){
						...
						$message_template['notice_setting_name'] = $v['name']??'';
        ...
        # 清除插件相关缓存（插件列表、钩子、语言）
        \app\common\logic\CacheLogic::clearPluginCache();
