...
class AuthModel extends Model
{
     ...
     * @author hh
     * @version v1
     * @param array authTitle - 权限标题
     * @param array authRuleName - 权限规则名称
     * @return int status - 状态码,200成功,400失败
     * @return string msg - 提示信息
     */
    public function deleteSystemAuth(array $authTitle, array $authRuleName = []): array
    {
        if(!empty($authTitle)){
            $authIds = $this
                ->where('title', 'IN', $authTitle)
                ->column('id');

            if(!empty($authIds)){
                $this->where('id', 'IN', $authIds)->delete();
        ...
        if(!empty($authRuleName)){
            $AuthRuleModel = new AuthRuleModel();
            $AuthRuleModel->where('name', 'IN', $authRuleName)->delete();
        }
